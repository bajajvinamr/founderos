...
function buildSourceConfig(): FounderOSConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("founderos-pr-432")).toBe(
      path.resolve(os.homedir(), "founderos-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("founderos/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://founderos.example", 3110)).toBe("https://founderos.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/founderos-feature",
      homeDir: "/tmp/founderos-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/founderos-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/founderos-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.FOUNDEROS_HOME).toBe(path.resolve("/tmp/founderos-worktrees"));
    expect(env.FOUNDEROS_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.FOUNDEROS_IN_WORKTREE).toBe("true");
    expect(env.FOUNDEROS_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.FOUNDEROS_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export FOUNDEROS_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-secrets-"));
    const originalInlineMasterKey = process.env.FOUNDEROS_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.FOUNDEROS_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.FOUNDEROS_SECRETS_MASTER_KEY;
      delete process.env.FOUNDEROS_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.FOUNDEROS_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.FOUNDEROS_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.FOUNDEROS_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          FOUNDEROS_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.FOUNDEROS_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.FOUNDEROS_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".founderos-worktrees"),
      });

      const envPath = path.join(repoRoot, ".founderos", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("FOUNDEROS_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("FOUNDEROS_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/FOUNDEROS_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.FOUNDEROS_AGENT_JWT_SECRET;
      } else {
        process.env.FOUNDEROS_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".founderos-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".founderos", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local FounderOS config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".founderos", "config.json");
    const originalCwd = process.cwd();
    const originalFounderOSConfig = process.env.FOUNDEROS_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalFounderOSConfig === undefined) {
        delete process.env.FOUNDEROS_CONFIG;
      } else {
        process.env.FOUNDEROS_CONFIG = originalFounderOSConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".founderos", "config.json");
    const envPath = path.join(worktreeRoot, ".founderos", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        envPath,
        [
          "FOUNDEROS_HOME=/tmp/founderos-worktrees",
          "FOUNDEROS_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".founderos", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".founderos", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local FounderOS instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".founderos-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalFounderOSConfig = process.env.FOUNDEROS_CONFIG;
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `FOUNDEROS_HOME=${homeDir}`,
          `FOUNDEROS_INSTANCE_ID=${currentInstanceId}`,
          "FOUNDEROS_WORKTREE_NAME=existing-name",
          "FOUNDEROS_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`FOUNDEROS_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("FOUNDEROS_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("FOUNDEROS_WORKTREE_COLOR=\"#112233\"");
    } finally {
      process.chdir(originalCwd);
      if (originalFounderOSConfig === undefined) {
        delete process.env.FOUNDEROS_CONFIG;
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".founderos-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `FOUNDEROS_HOME=${homeDir}\nFOUNDEROS_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`FOUNDEROS_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/founderos",
        targetRepoRoot: "/Users/example/founderos-pr-432",
        workspaceCwd: "/Users/example/founderos",
      }),
    ).toBe("/Users/example/founderos-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/founderos",
        targetRepoRoot: "/Users/example/founderos-pr-432",
        workspaceCwd: "/Users/example/founderos/packages/db",
      }),
    ).toBe("/Users/example/founderos-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "founderos-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("founderos-make-test", {
        seed: false,
        home: path.join(tempRoot, ".founderos-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".founderos", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".founderos", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("founderos-worktree-routines-");
