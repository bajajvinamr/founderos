...
import { ApiConnectionError, ApiRequestError, FounderOSApiClient } from "../client/http.js";

describe("FounderOSApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new FounderOSApiClient({
    ...
    expect(headers["x-founderos-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new FounderOSApiClient({ apiBase: "http://localhost:3100" });
