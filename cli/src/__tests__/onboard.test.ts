...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-onboard-"));
  const runtimeRoot = path.join(root, "runtime");
  const configPath = path.join(root, ".founderos", "config.json");
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-onboard-fresh-"));
  return path.join(root, ".founderos", "config.json");
}
...
describe("onboard", () => {
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.FOUNDEROS_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as FounderOSConfig;
  ...
  it("ignores deployment env overrides during --yes quickstart", async () => {
    const configPath = createFreshConfigPath();
    process.env.FOUNDEROS_DEPLOYMENT_MODE = "authenticated";
