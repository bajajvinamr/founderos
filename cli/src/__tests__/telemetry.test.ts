...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "founderos-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.FOUNDEROS_HOME = path.join(root, "home");
    process.env.FOUNDEROS_INSTANCE_ID = "telemetry-test";
