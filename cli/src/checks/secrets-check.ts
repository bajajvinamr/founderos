...
export function secretsCheck(config: FounderOSConfig, configPath?: string): CheckResult {
  ...
  if (provider !== "local_encrypted") {
    return {
      ...
      canRepair: false,
      repairHint: "Run `founderos configure --section secrets` and set provider to local_encrypted",
    };
  }

  const envMasterKey = process.env.FOUNDEROS_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    if (!decodeMasterKey(envMasterKey)) {
      return {
        ...
        status: "fail",
        message:
          "FOUNDEROS_SECRETS_MASTER_KEY is invalid (expected 32-byte base64, 64-char hex, or raw 32-char string)",
        canRepair: false,
        repairHint: "Set FOUNDEROS_SECRETS_MASTER_KEY to a valid key or unset it to use a key file",
      };
    }
    ...
    return withStrictModeNote(
      {
        name: "Secrets adapter",
        status: "pass",
        message: "Local encrypted provider configured via FOUNDEROS_SECRETS_MASTER_KEY",
      },
      config,
    );
  }

  const keyFileOverride = process.env.FOUNDEROS_SECRETS_MASTER_KEY_FILE;
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Check file permissions or set FOUNDEROS_SECRETS_MASTER_KEY",
    };
  }
