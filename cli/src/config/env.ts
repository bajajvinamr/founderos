...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by FounderOS CLI commands",
...
export function resolveFounderOSEnvFile(configPath?: string): string {
...
export function loadFounderOSEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeFounderOSEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readFounderOSEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeFounderOSEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeFounderOSEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readFounderOSEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeFounderOSEnvEntries(next, filePath);
  return next;
}
