...
type OnboardDefaults = Pick<FounderOSConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or FOUNDEROS_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "FOUNDEROS_PUBLIC_URL",
  "DATABASE_URL",
  "FOUNDEROS_DB_BACKUP_ENABLED",
  "FOUNDEROS_DB_BACKUP_INTERVAL_MINUTES",
  "FOUNDEROS_DB_BACKUP_RETENTION_DAYS",
  "FOUNDEROS_DB_BACKUP_DIR",
  "FOUNDEROS_DEPLOYMENT_MODE",
  "FOUNDEROS_DEPLOYMENT_EXPOSURE",
  "FOUNDEROS_BIND",
  "FOUNDEROS_BIND_HOST",
  "FOUNDEROS_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "FOUNDEROS_ALLOWED_HOSTNAMES",
  "FOUNDEROS_AUTH_BASE_URL_MODE",
  "FOUNDEROS_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "FOUNDEROS_STORAGE_PROVIDER",
  "FOUNDEROS_STORAGE_LOCAL_DIR",
  "FOUNDEROS_STORAGE_S3_BUCKET",
  "FOUNDEROS_STORAGE_S3_REGION",
  "FOUNDEROS_STORAGE_S3_ENDPOINT",
  "FOUNDEROS_STORAGE_S3_PREFIX",
  "FOUNDEROS_STORAGE_S3_FORCE_PATH_STYLE",
  "FOUNDEROS_SECRETS_PROVIDER",
  "FOUNDEROS_SECRETS_STRICT_MODE",
  "FOUNDEROS_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<FounderOSConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const instanceId = resolveFounderOSInstanceId();
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.FOUNDEROS_PUBLIC_URL?.trim() ||
      process.env.FOUNDEROS_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.FOUNDEROS_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.FOUNDEROS_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.FOUNDEROS_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.FOUNDEROS_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.FOUNDEROS_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.FOUNDEROS_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.FOUNDEROS_ALLOWED_HOSTNAMES
    ? process.env.FOUNDEROS_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.FOUNDEROS_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.FOUNDEROS_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.FOUNDEROS_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.FOUNDEROS_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.FOUNDEROS_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.FOUNDEROS_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.FOUNDEROS_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.FOUNDEROS_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.FOUNDEROS_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.FOUNDEROS_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.FOUNDEROS_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.FOUNDEROS_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.FOUNDEROS_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.FOUNDEROS_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.FOUNDEROS_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "FOUNDEROS_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.FOUNDEROS_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "FOUNDEROS_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.FOUNDEROS_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "FOUNDEROS_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<FounderOSConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" founderos onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveFounderOSInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: FounderOSConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing FounderOS install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("founderos configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("FOUNDEROS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.FOUNDEROS_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("FOUNDEROS_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("FOUNDEROS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: FOUNDEROS_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("founderos run")}`,
        `Reconfigure later: ${pc.cyan("founderos configure")}`,
        `Diagnose setup: ${pc.cyan("founderos doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.FOUNDEROS_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing FounderOS setup is ready.");
    return;
  }
  ...
  let llm: FounderOSConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@founderos/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `founderos doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("founderos run")}`,
        `Then: ${pc.cyan("founderos auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
