...
import { ensureAgentJwtSecret, loadFounderOSEnvFile, mergeFounderOSEnvEntries, readFounderOSEnvEntries, resolveFounderOSEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { FounderOSConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printFounderOSCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.FOUNDEROS_CONFIG;
...
const WORKTREE_NAME_PREFIX = "founderos-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.FOUNDEROS_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.founderos"));
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readFounderOSEnvEntries(resolveFounderOSEnvFile(input.configPath));
  const homeDir = nonEmpty(envEntries.FOUNDEROS_HOME);
  const instanceId = nonEmpty(envEntries.FOUNDEROS_INSTANCE_ID);

  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local FounderOS instance. Expected FOUNDEROS_HOME and FOUNDEROS_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveSourceConnectionString(config: FounderOSConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://founderos:founderos@127.0.0.1:${port}/founderos`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: FounderOSConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.FOUNDEROS_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.FOUNDEROS_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.FOUNDEROS_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.FOUNDEROS_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: FounderOSConfig;
  targetConfig: FounderOSConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveFounderOSEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readFounderOSEnvEntries(sourceEnvFile);
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://founderos:founderos@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "founderos");
    }
    ...
    const adminConnectionString = `postgres://founderos:founderos@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "founderos");
    const targetConnectionString = `postgres://founderos:founderos@127.0.0.1:${targetHandle.port}/founderos`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readFounderOSEnvEntries(resolveFounderOSEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.FOUNDEROS_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.FOUNDEROS_AGENT_JWT_SECRET);
  mergeFounderOSEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { FOUNDEROS_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadFounderOSEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run FounderOS inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printFounderOSCliBanner();
  p.intro(pc.bgCyan(pc.black(" founderos worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printFounderOSCliBanner();
  p.intro(pc.bgCyan(pc.black(" founderos worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasFounderOSConfig: existsSync(path.resolve(worktreePath, ".founderos", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printFounderOSCliBanner();
  p.intro(pc.bgCyan(pc.black(" founderos worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveFounderOSEnvFile(configPath);
  const envEntries = readFounderOSEnvEntries(envPath);
  const out = {
    FOUNDEROS_CONFIG: configPath,
    ...(envEntries.FOUNDEROS_HOME ? { FOUNDEROS_HOME: envEntries.FOUNDEROS_HOME } : {}),
    ...(envEntries.FOUNDEROS_INSTANCE_ID ? { FOUNDEROS_INSTANCE_ID: envEntries.FOUNDEROS_INSTANCE_ID } : {}),
    ...(envEntries.FOUNDEROS_CONTEXT ? { FOUNDEROS_CONTEXT: envEntries.FOUNDEROS_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasFounderOSConfig)
      .map((choice) => path.resolve(choice.worktree, ".founderos", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readFounderOSEnvEntries(resolveFounderOSEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start FounderOS once) before using worktree merge history.`,
      );
    }
...
function resolveRunningEmbeddedPostgresPid(config: FounderOSConfig): number | null {
  if (config.database.mode !== "embedded-postgres") {
    return null;
  }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasFounderOSConfig ? "founderos" : "no-founderos-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".founderos", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".founderos", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .founderos/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasFounderOSConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a FounderOS worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasFounderOSConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No FounderOS worktrees were found. Run `founderos worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target FounderOS configs are the same. Choose different --from/--to worktrees.");
  }
...
export async function worktreeReseedCommand(opts: WorktreeReseedOptions): Promise<void> {
  printFounderOSCliBanner();
  p.intro(pc.bgCyan(pc.black(" founderos worktree reseed ")));
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target FounderOS configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop FounderOS in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated FounderOS DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local FounderOS instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated FounderOS instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with founderos- if needed (created at ~/founderos-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: FOUNDEROS_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: FOUNDEROS_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source FOUNDEROS_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local FounderOS instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like FounderOS worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another FounderOS instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source FOUNDEROS_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with founderos- if needed")
