...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@founderos/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://founderos:founderos@127.0.0.1:${port}/founderos`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" founderos db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveFounderOSInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "founderos";
