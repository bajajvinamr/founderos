...
import { ApiRequestError, FounderOSApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to FounderOS config file")
    .option("-d, --data-dir <path>", "FounderOS data directory root (isolates state from ~/.founderos)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the FounderOS API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.FOUNDEROS_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.FOUNDEROS_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.FOUNDEROS_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set FOUNDEROS_COMPANY_ID, or set context profile companyId via `founderos context set`.",
    );
  }

  const api = new FounderOSApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.FOUNDEROS_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.FOUNDEROS_SERVER_PORT || "");
