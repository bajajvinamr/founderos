...
export const DEFAULT_WORKTREE_HOME = "~/.founderos-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".founderos");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: FounderOSConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "founderos",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    FOUNDEROS_HOME: paths.homeDir,
    FOUNDEROS_INSTANCE_ID: paths.instanceId,
    FOUNDEROS_CONFIG: paths.configPath,
    FOUNDEROS_CONTEXT: paths.contextPath,
    FOUNDEROS_IN_WORKTREE: "true",
    ...(branding?.name ? { FOUNDEROS_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { FOUNDEROS_WORKTREE_COLOR: branding.color } : {}),
  };
}
