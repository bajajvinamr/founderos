...
export class FounderOSApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-founderos-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the FounderOS API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the FounderOS server is not running, the configured URL is wrong, or the request is being blocked before it reaches FounderOS.",
    "",
    "Try:",
    "- Start FounderOS with `pnpm dev` or `pnpm founderos run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If FounderOS is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`FOUNDEROS_API_URL\`.`,
  );
  return lines.join("\n");
}
