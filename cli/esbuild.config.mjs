/**
 * esbuild configuration for building the founderos CLI for npm.
 *
 * Bundles all workspace packages (@founderos/*) into a single file.
...
// to npm and resolved at runtime (e.g. @founderos/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@founderos/")) {
      externals.add(name);
    }
  }
