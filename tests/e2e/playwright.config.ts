...
const PORT = Number(process.env.FOUNDEROS_E2E_PORT ?? 3199);
...
export default defineConfig({
  ...
  // The webServer directive starts `founderos run` before tests.
  // Expects `pnpm founderos` to be runnable from repo root.
  webServer: {
    command: `pnpm founderos run`,
