...
import type { AdapterModel } from "@founderos/adapter-utils";
import { asString, runChildProcess } from "@founderos/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.FOUNDEROS_PI_COMMAND === "string" &&
    process.env.FOUNDEROS_PI_COMMAND.trim().length > 0
      ? process.env.FOUNDEROS_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["FOUNDEROS_", "npm_", "NPM_"] as const;
