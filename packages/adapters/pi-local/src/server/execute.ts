...
import { inferOpenAiCompatibleBiller, type AdapterExecutionContext, type AdapterExecutionResult } from "@founderos/adapter-utils";
import {
  asString,
  asNumber,
  asStringArray,
...
const FOUNDEROS_SESSIONS_DIR = path.join(os.homedir(), ".pi", "foundeross");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[founderos] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[founderos] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[founderos] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(FOUNDEROS_SESSIONS_DIR, { recursive: true });
  return FOUNDEROS_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(FOUNDEROS_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Inject skills
  const piSkillEntries = await readFounderOSRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveFounderOSDesiredSkillNames(config, piSkillEntries);
  ...
  if (workspaceCwd) env.FOUNDEROS_WORKSPACE_CWD = workspaceCwd;
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[founderos] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[founderos] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const renderedBootstrapPrompt =
    !canResumeSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderFounderOSWakePrompt(context.founderosWake, { resumedSession: canResumeSession });
  ...
  const buildArgs = (sessionFile: string): string[] => {
    ...
    // Add FounderOS skills directory so Pi can load the founderos skill
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[founderos] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
