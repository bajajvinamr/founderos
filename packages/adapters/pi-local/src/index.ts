...
Use when:
- You want FounderOS to run Pi (the AI coding agent) locally as the agent runtime
...
- FounderOS requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/foundeross/ and resumed with --session.
