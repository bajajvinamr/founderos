/**
 * Honest-disable tests for codex-local: testEnvironment() returns "warn"
 * with code "codex_local_cli_not_found" when the `codex` binary is not in PATH.
 *
...
vi.mock("node:child_process", async (importOriginal) => {
  ...
  return {
    ...
    execSync: vi.fn((cmd: string) => {
      // Simulate "which codex: not found" for the binary availability check.
...
describe("testEnvironment — CLI binary not in PATH", () => {
  it("returns status \"warn\" with code \"codex_local_cli_not_found\"", async () => {
    ...
    expect(result.checks[0].message).toContain("Codex CLI");
