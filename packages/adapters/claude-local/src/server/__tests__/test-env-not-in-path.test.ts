/**
 * Honest-disable tests for claude-local: testEnvironment() returns "warn"
 * with code "claude_local_cli_not_found" when the `claude` binary is not in PATH.
 *
...
vi.mock("node:child_process", async (importOriginal) => {
  ...
  return {
    ...
    execSync: vi.fn((cmd: string) => {
      // Simulate "which claude: not found" for the binary availability check.
...
describe("testEnvironment — CLI binary not in PATH", () => {
  it("returns status \"warn\" with code \"claude_local_cli_not_found\"", async () => {
    const result = await testEnvironment(makeCtx());

    expect(result.status).toBe("warn");
    expect(result.checks).toHaveLength(1);
    expect(result.checks[0].code).toBe("claude_local_cli_not_found");
    expect(result.checks[0].level).toBe("warn");
    expect(result.checks[0].message).toContain("Claude Code CLI");
