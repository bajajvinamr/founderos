...
import { ensureFounderOSSkillSymlink, type FounderOSSkillEntry } from "@founderos/adapter-utils/server-utils";

const DEFAULT_FOUNDEROS_INSTANCE_ID = "default";

type SkillEntry = FounderOSSkillEntry;
...
function resolveManagedClaudePromptCacheRoot(
  ...
  const founderosHome = nonEmpty(env.FOUNDEROS_HOME) ?? path.resolve(os.homedir(), ".founderos");
  const instanceId = nonEmpty(env.FOUNDEROS_INSTANCE_ID) ?? DEFAULT_FOUNDEROS_INSTANCE_ID;
  return path.resolve(
    founderosHome,
    "instances",
    instanceId,
...
async function buildClaudePromptBundleKey(input: {
  ...
  hash.update("founderos-claude-prompt-bundle:v1\n");
...
export async function prepareClaudePromptBundle(input: {
  ...
  for (const entry of skills) {
    ...
    try {
      await ensureFounderOSSkillSymlink(entry.source, target);
    } catch (err) {
      await onLog(
        "stderr",
        `[founderos] Failed to materialize Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
