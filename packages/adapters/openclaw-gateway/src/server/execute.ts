...
const DEFAULT_CLIENT_VERSION = "founderos";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "founderos";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`founderos:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`founderos:issue:${input.issueId}`, input.agentId);
  }
...
function resolveFounderOSApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/founderos-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildFounderOSEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const founderosApiUrlOverride = resolveFounderOSApiUrlOverride(ctx.config.founderosApiUrl);
  const founderosEnv: Record<string, string> = {
    ...buildFounderOSEnv(ctx.agent),
    FOUNDEROS_RUN_ID: ctx.runId,
  };

  if (founderosApiUrlOverride) {
    founderosEnv.FOUNDEROS_API_URL = founderosApiUrlOverride;
  }
  if (wakePayload.taskId) founderosEnv.FOUNDEROS_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) founderosEnv.FOUNDEROS_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) founderosEnv.FOUNDEROS_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) founderosEnv.FOUNDEROS_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) founderosEnv.FOUNDEROS_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    founderosEnv.FOUNDEROS_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  founderosEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/founderos-claimed-api-key.json";
  const orderedKeys = [
    "FOUNDEROS_RUN_ID",
    "FOUNDEROS_AGENT_ID",
    "FOUNDEROS_COMPANY_ID",
    "FOUNDEROS_API_URL",
    "FOUNDEROS_TASK_ID",
    "FOUNDEROS_WAKE_REASON",
    "FOUNDEROS_WAKE_COMMENT_ID",
    "FOUNDEROS_APPROVAL_ID",
    "FOUNDEROS_APPROVAL_STATUS",
    "FOUNDEROS_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = founderosEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = founderosEnv.FOUNDEROS_API_URL ?? "<set FOUNDEROS_API_URL>";

  const lines = [
    "FounderOS wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `FOUNDEROS_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load FOUNDEROS_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $FOUNDEROS_API_KEY on every API call.",
    "- Use X-FounderOS-Run-Id: $FOUNDEROS_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: FOUNDEROS_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$FOUNDEROS_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$FOUNDEROS_COMPANY_ID/issues?assigneeAgentId=$FOUNDEROS_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardFounderOSPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  founderosEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateFounderOS = parseObject(payloadTemplate.founderos);
  const workspace = asRecord(ctx.context.founderosWorkspace);
  const workspaces = Array.isArray(ctx.context.founderosWorkspaces)
    ? ctx.context.founderosWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.founderosRuntimeServiceIntents)
    ? ctx.context.founderosRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardFounderOS: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: founderosEnv.FOUNDEROS_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.founderosWake);
  if (Object.keys(structuredWake).length > 0) {
    standardFounderOS.wake = structuredWake;
  }

  if (workspace) {
    standardFounderOS.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardFounderOS.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardFounderOS.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "founderos-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const founderosEnv = buildFounderOSEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderFounderOSWakePrompt(ctx.context.founderosWake);
  const structuredWakeJson = stringifyFounderOSWakePayload(ctx.context.founderosWake);
  ...
  const founderosPayload = buildStandardFounderOSPayload(ctx, wakePayload, founderosEnv, payloadTemplate);
  ...
  agentParams.founderos = founderosPayload;
