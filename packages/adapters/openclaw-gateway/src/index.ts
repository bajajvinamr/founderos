...
Use when:
- You want FounderOS to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the FounderOS server.
...
- founderosApiUrl (string, optional): absolute FounderOS base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/founderos-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default founderos)

Standard outbound payload additions:
- founderos (object): standardized FounderOS context added to every gateway agent request
- founderos.workspace (object, optional): resolved execution workspace for this run
- founderos.workspaces (array, optional): additional workspace hints FounderOS exposed to the run
- founderos.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
