...
This document describes how `@founderos/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into FounderOS logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (FounderOS `runId`)
