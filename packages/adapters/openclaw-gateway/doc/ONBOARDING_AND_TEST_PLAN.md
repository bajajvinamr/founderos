...
## Scope
This plan is now **gateway-only**. FounderOS supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses FounderOS skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a FounderOS task.
...
```bash
pnpm --filter @founderos/server test:run -- openclaw-gateway
pnpm --filter @founderos/server typecheck
pnpm --filter @founderos/ui typecheck
pnpm --filter founderos typecheck
```
