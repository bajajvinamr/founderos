/**
 * Honest-disable tests for gemini-local: testEnvironment() returns "warn"
 * with code "gemini_local_cli_not_found" when the `gemini` binary is not in PATH.
 *
...
vi.mock("node:child_process", async (importOriginal) => {
  ...
  return {
    ...
    execSync: vi.fn((cmd: string) => {
      // Simulate "which gemini: not found" for the binary availability check.
...
describe("testEnvironment — CLI binary not in PATH", () => {
  it("returns status \"warn\" with code \"gemini_local_cli_not_found\"", async () => {
    const result = await testEnvironment(makeCtx());

    expect(result.status).toBe("warn");
    expect(result.checks).toHaveLength(1);
    expect(result.checks[0].code).toBe("gemini_local_cli_not_found");
    expect(result.checks[0].level).toBe("warn");
    expect(result.checks[0].message).toContain("Gemini CLI");
