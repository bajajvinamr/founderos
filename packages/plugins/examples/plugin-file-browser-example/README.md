...
Example FounderOS plugin that demonstrates:
...
```bash
pnpm --filter @founderos/plugin-file-browser-example build
pnpm founderos plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm founderos plugin uninstall founderos-file-browser-example --force
```
...
- Optional: use `founderos-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
