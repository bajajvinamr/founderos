/**
 * Core types for the FounderOS plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@founderos/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @founderos/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of FounderOS object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the FounderOS UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit FounderOS domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core FounderOS domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the FounderOS secret provider
...
export interface PluginSecretsClient {
   ...
   * in the FounderOS secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@founderos/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@founderos/plugin-sdk";
 *
