...
import type { FounderOSPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@founderos/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: FounderOSPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@founderos/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
