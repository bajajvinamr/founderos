 ...
 * channel.
 *
 * The FounderOS plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@founderos/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all FounderOS messages.
 */
...
/**
 * FounderOS plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this FounderOS instance. */
    instanceId: string;
    /** Semver version of the running FounderOS host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
