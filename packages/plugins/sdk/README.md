# `@founderos/plugin-sdk`

Official TypeScript SDK for FounderOS plugin authors.

- **Worker SDK:** `@founderos/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@founderos/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@founderos/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@founderos/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@founderos/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@founderos/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@founderos/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@founderos/plugin-sdk/ui/hooks` | Hooks only |
| `@founderos/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@founderos/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@founderos/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@founderos/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@founderos/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@founderos/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main FounderOS app. They can call ordinary FounderOS HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the FounderOS instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@founderos/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@founderos/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core FounderOS information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@founderos/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@founderos/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@founderos/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@founderos/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@founderos/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@founderos/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@founderos/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@founderos/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@founderos/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@founderos/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@founderos/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@founderos/plugin-sdk/bundlers";
...
```bash
founderos-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@founderos/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__founderos__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__founderos__/events` streams `reload` SSE events on UI build changes
