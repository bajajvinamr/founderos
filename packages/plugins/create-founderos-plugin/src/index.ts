...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".founderos-sdk");
...
/**
 * Generate a complete FounderOS plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A FounderOS plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "founderos-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["founderos", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@founderos/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@founderos/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@founderos/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@founderos/plugin-sdk\` and \`@founderos/shared\` from a local FounderOS checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.founderos-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-founderos-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <founderos-sdk-path>]");
    process.exit(1);
  }
