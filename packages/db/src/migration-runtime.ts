...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://founderos:founderos@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "founderos");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://founderos:founderos@127.0.0.1:${preferredPort}/founderos`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://founderos:founderos@127.0.0.1:${port}/founderos`,
  ...
  const adminConnectionString = `postgres://founderos:founderos@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "founderos");

  return {
    connectionString: `postgres://founderos:founderos@127.0.0.1:${selectedPort}/founderos`,
