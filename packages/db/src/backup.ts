...
function resolveFounderOSInstanceId(): string {
  const raw = process.env.FOUNDEROS_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveFounderOSHomeDir(), "instances", resolveFounderOSInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveFounderOSHomeDir(), "instances", resolveFounderOSInstanceId(), "data", "backups");
}
