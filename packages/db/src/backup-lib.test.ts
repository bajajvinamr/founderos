...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("founderos-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("founderos-buffered-writer-");
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  ...
  it.skip(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        ...
        "founderos_restore_target",
      );
      const backupDir = createTempDir("founderos-db-backup-output-");
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          filenamePrefix: "founderos-test",
        });

        expect(result.backupFile).toMatch(/founderos-test-.*\.sql\.gz$/);
  ...
  it(
    ...
    async () => {
      ...
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const backupDir = createTempDir("founderos-db-restore-manual-");
      ...
      try {
        await fs.promises.writeFile(
          backupFile,
          [
            "-- FounderOS database backup",
            "-- Created: 2026-04-06T00:00:00.000Z",
            "",
            "BEGIN;",
            "-- founderos statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
