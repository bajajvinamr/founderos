...
const FOUNDEROS_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface FounderOSSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: FounderOSSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "founderos_required",
      originLabel: "Required by FounderOS",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by FounderOS",
    readOnly: false,
  };
}
...
type FounderOSWakeExecutionPrincipal = {
  type: "agent" | "user" | null;
  agentId: string | null;
  userId: string | null;
};

type FounderOSWakeExecutionStage = {
  wakeRole: "reviewer" | "approver" | "executor" | null;
  stageId: string | null;
  stageType: string | null;
  currentParticipant: FounderOSWakeExecutionPrincipal | null;
  returnAssignee: FounderOSWakeExecutionPrincipal | null;
  lastDecisionOutcome: string | null;
  allowedActions: string[];
};

type FounderOSWakeComment = {
  id: string | null;
  issueId: string | null;
  body: string;
...
type FounderOSWakePayload = {
  reason: string | null;
  issue: FounderOSWakeIssue | null;
  executionStage: FounderOSWakeExecutionStage | null;
  commentIds: string[];
  latestCommentId: string | null;
  comments: FounderOSWakeComment[];
...
function normalizeFounderOSWakeIssue(value: unknown): FounderOSWakeIssue | null {
...
function normalizeFounderOSWakeComment(value: unknown): FounderOSWakeComment | null {
...
function normalizeFounderOSWakeExecutionPrincipal(value: unknown): FounderOSWakeExecutionPrincipal | null {
...
function normalizeFounderOSWakeExecutionStage(value: unknown): FounderOSWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizeFounderOSWakeExecutionPrincipal(stage.currentParticipant);
  const returnAssignee = normalizeFounderOSWakeExecutionPrincipal(stage.returnAssignee);
...
export function normalizeFounderOSWakePayload(value: unknown): FounderOSWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeFounderOSWakeComment(entry))
        .filter((entry): entry is FounderOSWakeComment => Boolean(entry))
    : [];
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizeFounderOSWakeExecutionStage(payload.executionStage);

  if (comments.length === 0 && commentIds.length === 0 && !executionStage && !normalizeFounderOSWakeIssue(payload.issue)) {
    return null;
  }

  return {
    reason: asString(payload.reason, "").trim() || null,
    issue: normalizeFounderOSWakeIssue(payload.issue),
    executionStage,
    commentIds,
...
export function stringifyFounderOSWakePayload(value: unknown): string | null {
  const normalized = normalizeFounderOSWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function renderFounderOSWakePrompt(
  value: unknown,
  ...
  const principalLabel = (principal: FounderOSWakeExecutionPrincipal | null) => {
  ...
  const lines = resumedSession
      ? [
        "## FounderOS Resume Delta",
        "",
        "You are resuming an existing FounderOS session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "FOUNDEROS_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildFounderOSEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    FOUNDEROS_AGENT_ID: agent.id,
    FOUNDEROS_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.FOUNDEROS_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.FOUNDEROS_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.FOUNDEROS_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.FOUNDEROS_API_URL = apiUrl;
  return vars;
}
...
export async function resolveFounderOSSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...FOUNDEROS_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listFounderOSSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<FounderOSSkillEntry[]> {
  const root = await resolveFounderOSSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `founderos-ai/founderos/${entry.name}`,
        runtimeName: entry.name,
        source: path.join(root, entry.name),
        required: true,
        requiredReason: "Bundled FounderOS skills are always available for local adapters.",
      }));
  } catch {
    return [];
  }
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the FounderOS skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "FounderOS cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredFounderOSRuntimeSkills(value: unknown): FounderOSSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: FounderOSSkillEntry[] = [];
...
export async function readFounderOSRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<FounderOSSkillEntry[]> {
  const configuredEntries = normalizeConfiguredFounderOSRuntimeSkills(config.founderosRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listFounderOSSkillEntries(moduleDir, additionalCandidates);
}

export async function readFounderOSSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listFounderOSSkillEntries(moduleDir);
...
export function readFounderOSSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.founderosSkillSync;
...
function canonicalizeDesiredFounderOSSkillReference(
  reference: string,
...
export function resolveFounderOSDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readFounderOSSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredFounderOSSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeFounderOSSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.founderosSkillSync;
  const current =
  ...
  next.founderosSkillSync = current;
  return next;
}

export async function ensureFounderOSSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the FounderOS server itself is started from
    // within a Claude Code session (e.g. `npx founderos run` in a terminal
