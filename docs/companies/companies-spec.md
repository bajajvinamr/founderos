...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only FounderOS.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.founderos.yaml`
...
- FounderOS must not require extra top-level fields for skill validity
- FounderOS-specific extensions must live under `metadata.founderos` or `metadata.sources`
...
For FounderOS, the preferred fidelity extension is:
...
- issue/task FounderOS-only metadata
...
- FounderOS tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested FounderOS shape:
...
Additional rules for FounderOS exporters:
...
- omit empty and default-valued FounderOS fields when possible
...
- FounderOS exporters should emit `.founderos.yaml` as a sidecar by default
...
## 19. FounderOS Mapping

FounderOS can map this spec to its runtime model like this:
...
- FounderOS extension:
  - `.founderos.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other FounderOS-specific fidelity

Inline FounderOS-only metadata that must live inside a shared markdown file should use:

- `metadata.founderos`

That keeps the base format broader than FounderOS.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only FounderOS.

## 20. Cutover

FounderOS should cut over to this markdown-first package model as the primary portability format.

`founderos.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For FounderOS, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
