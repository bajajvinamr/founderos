...
│  FounderOS Server  │◄────────────────────────────────│  uiParserCache    │
│  (in-memory)      │                                 └──────────────────┘
└────────┬─────────┘
         │ serves JS to browser
         ▼
┌──────────────────┐   fetch() + eval   ┌──────────────────┐
│  FounderOS UI     │─────────────────────→│  parseStdoutLine │
...
### 1. `founderos.adapterUiParser` — contract version
...
The FounderOS host checks this field. If the major version is unsupported, the host logs a warning and falls back to the generic parser instead of executing potentially incompatible code.
