...
- Running a Python script that calls the FounderOS API
...
1. FounderOS spawns the configured command as a child process
2. Standard FounderOS environment variables are injected (`FOUNDEROS_AGENT_ID`, `FOUNDEROS_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the FounderOS API and perform work.
