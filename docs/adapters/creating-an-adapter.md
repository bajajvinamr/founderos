...
Build a custom adapter to connect FounderOS to any agent runtime.
...
|---|---|---|
| Source | Inside `founderos-fork` | Separate npm package |
| Distribution | Ships with FounderOS | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying FounderOS's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@founderos/adapter-utils/server-utils`
2. Build environment with `buildFounderOSEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@founderos/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildFounderOSEnv(agent)` | `@founderos/adapter-utils/server-utils` | Inject `FOUNDEROS_*` env vars |
| `renderTemplate(tpl, data)` | `@founderos/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@founderos/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@founderos/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in FounderOS's source:
...
`format-event.ts` — pretty-prints stdout for `founderos run --watch` using `picocolors`.
...
Make FounderOS skills discoverable to your agent runtime without writing to the agent's working directory:
