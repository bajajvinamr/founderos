...
summary: Build, package, and distribute adapters as plugins without modifying FounderOS source
---

FounderOS supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to FounderOS's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `founderos-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with FounderOS | Published to npm or linked via `file:` |
| Updates | Requires FounderOS release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@founderos/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `founderos.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
export async function execute(
  ...
  // 2. Build environment with FounderOS vars injected
  const env = buildFounderOSEnv(agent);
...
#### Available Helpers from `@founderos/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildFounderOSEnv(agent)` | Inject `FOUNDEROS_*` environment variables |
...
```sh
# Via the FounderOS UI
# Settings → Adapters → Install from npm → "my-founderos-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-founderos-adapter"}'
```
...
Local adapters are symlinked into FounderOS's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.founderos/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-founderos-adapter",
...
```ts
import type { AdapterSessionCodec } from "@founderos/adapter-utils";
...
Other FounderOS users can then install your adapter by package name from the UI or API.
