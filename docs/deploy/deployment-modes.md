...
FounderOS supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm founderos allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, FounderOS emits a one-time claim URL at startup:
...
```sh
FOUNDEROS_DEPLOYMENT_MODE=authenticated FOUNDEROS_BIND=lan pnpm founderos run
```
