...
FounderOS companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .founderos.yaml     # Adapter config, env inputs, routines
```
...
- **.founderos.yaml** holds FounderOS-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
founderos company export <company-id> --out ./my-export
```
...
founderos company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
founderos company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
founderos company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.founderos.yaml`
...
```sh
# From a local folder
founderos company import ./my-export

# From a GitHub URL
founderos company import https://github.com/org/repo

# From a GitHub subfolder
founderos company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
founderos company import org/repo
founderos company import org/repo/companies/acme
```
...
If `--target` is not specified, FounderOS infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
founderos company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
founderos company import org/company-templates/engineering-team \
  --target new \
...
```sh
founderos company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
founderos company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
founderos company import ./package \
  --target new \
  --yes \
  --json
```
...
FounderOS supports several GitHub URL formats:
