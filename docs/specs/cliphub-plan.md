# ClipHub: Marketplace for FounderOS Team Configurations
...
> The "app store" for whole-company AI teams — pre-built FounderOS configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for FounderOS-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | FounderOS company (agents, projects, governance) |
...
A complete FounderOS company configuration:
...
Single-agent configurations designed to plug into a FounderOS org:
...
Portable skill files that any FounderOS agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with FounderOS's skill loading system
...
interface Listing {
  ...
  founderosVersionMin: string;     // Minimum FounderOS version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target FounderOS company
...
| `POST` | `/api/listings/:id/install` | Install to FounderOS company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target FounderOS company (or create new)
  → ClipHub API calls FounderOS API to:
  ...
  → Redirect to FounderOS dashboard with new team running
```
...
### 5.3 Creator: Export from FounderOS → Publish

```
Running FounderOS company → "Export as Blueprint" (CLI or UI)
  → FounderOS exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (FounderOS brand blue/purple)
...
| Install flow | One-click deploy to FounderOS company |
...
{
  "targetCompanyId": "uuid",         // Existing FounderOS company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `founderos-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as FounderOS
- **Backend**: Node.js API (or extend FounderOS server)
- **Database**: Postgres (can share FounderOS's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with FounderOS auth (or OAuth2)

### 9.2 Integration with FounderOS

ClipHub can be:
- **Option A**: A separate app that calls FounderOS's API to install blueprints
- **Option B**: A built-in section of the FounderOS UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing FounderOS UI and API.
...
- [ ] Install flow: blueprint → FounderOS company
...
- [ ] "Export from FounderOS" CLI command
