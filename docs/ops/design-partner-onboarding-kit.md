...
- [ ] **Webhook signature verification is wired correctly.** Handler lives at `server/src/routes/billing.ts:165` (POST `/api/billing/webhook`); raw body is captured via `express.json({verify})` at `server/src/app.ts:186-192`, which attaches `req.rawBody = buf` as a side-effect of JSON parsing. The handler then reads `req.rawBody` and calls `stripeClient.constructWebhookEvent(rawBody, signature, endpointSecret)` at `billing.ts:190`. This is load-bearing — if the verify hook is removed or the handler reads `req.body` instead of `req.rawBody`, signature verification fails silently on every event. (Previously the kit cited an `express.raw` ordering pattern; the actual implementation uses the cleaner verify-hook approach.)
