...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-FounderOS-Signature` + `X-FounderOS-Timestamp` header pair matching the trigger's signing mode.
