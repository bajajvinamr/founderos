...
- **GitHub Actions billing is exhausted at the account/org level — ALL CI workflows have been broken since 2026-05-02T01:32:07Z.** Originally noted as `release-main.yml` only; verified 2026-05-05 that the scope is wider — every workflow (CI, PR Lint, NPM Audit, Gitleaks, CodeQL, E2E, PR Info) fails at the runner-allocation phase. Signature: "install + cache" job completes in 9-15 seconds with empty `steps[]`, log blob returns `BlobNotFound` 404, all workflows fail simultaneously within seconds of each other. The actual prod deploy path is `.github/workflows/deploy-prod.yml`. Local `pnpm install --frozen-lockfile` and `pnpm typecheck` succeed cleanly — code is fine, infra is the blocker. Treat `deploy-prod.yml` as the source of truth until billing unblocks. Per the user's CI-broken guardrail, **do NOT ship features over this** — diagnose, escalate, hold.
- **`/api/health/deep` is admin-gated** (closed 2026-05-03 council; verified 2026-05-05 at `health.ts:132-133` via `assertInstanceAdmin`). For deploy probes use `/api/readyz` (public, returns 200 "ready"). For oncall dashboards, hit `/api/health/deep` with an instance-admin credential. Note: `/api/health` ROOT is still public and exposes `deploymentMode/Exposure/authReady/bootstrapStatus/devServer` — task #139 strips that to `{ok, version}`.
