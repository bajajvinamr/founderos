...
base_cwd="${FOUNDEROS_WORKSPACE_BASE_CWD:?FOUNDEROS_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${FOUNDEROS_WORKSPACE_CWD:?FOUNDEROS_WORKSPACE_CWD is required}"
founderos_home="${FOUNDEROS_HOME:-$HOME/.founderos}"
founderos_instance_id="${FOUNDEROS_INSTANCE_ID:-default}"
founderos_dir="$worktree_cwd/.founderos"
worktree_config_path="$founderos_dir/config.json"
worktree_env_path="$founderos_dir/.env"
worktree_name="${FOUNDEROS_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${FOUNDEROS_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.founderos/config.json" || -L "$base_cwd/.founderos/config.json" ) ]]; then
  source_config_path="$base_cwd/.founderos/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$founderos_home/instances/$founderos_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$founderos_dir"

run_founderos_command() {
  local command_args=("$@")
  if command -v pnpm >/dev/null 2>&1 && pnpm founderos --help >/dev/null 2>&1; then
    pnpm founderos "${command_args[@]}"
    return 0
  fi
  ...
  if command -v founderos >/dev/null 2>&1; then
    founderos "${command_args[@]}"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  FOUNDEROS_DIR="$founderos_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  FOUNDEROS_WORKTREES_DIR="${FOUNDEROS_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const founderosDir = process.env.FOUNDEROS_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.FOUNDEROS_WORKTREES_DIR) ?? "~/.founderos-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(founderosDir, "config.json");
  const envPath = path.resolve(founderosDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "founderos",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.FOUNDEROS_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.FOUNDEROS_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.FOUNDEROS_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "FOUNDEROS_HOME=" + JSON.stringify(worktreeHome),
    "FOUNDEROS_INSTANCE_ID=" + JSON.stringify(instanceId),
    "FOUNDEROS_CONFIG=" + JSON.stringify(configPath),
    "FOUNDEROS_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "FOUNDEROS_IN_WORKTREE=true",
    "FOUNDEROS_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.FOUNDEROS_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("FOUNDEROS_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "founderos CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".founderos-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
