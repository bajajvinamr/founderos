  ...
  await db.transaction(async (tx) => {
    ...
    for (const tableName of MIRA_LABS_PURGE_TABLES) {
      ...
      const result = await tx.execute(
        sql`DELETE FROM ${sql.identifier(tableName)} WHERE ${sql.identifier(fkColumn)} = ${companyId}::uuid`,
      );
      const rowCount = (result as { rowCount?: number }).rowCount ?? 0;
      if (rowCount > 0) {
        console.log(`[seed-mira-labs]   purged ${rowCount} row(s) from ${tableName}`);
    ...
    const companyRowCount = (companyResult as { rowCount?: number }).rowCount ?? 0;
    totalDeleted += companyRowCount;
    console.log(
