...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[founderos] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`founderos worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".founderos", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".founderos/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[founderos] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[founderos] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[founderos] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  FOUNDEROS_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.FOUNDEROS_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.FOUNDEROS_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  env.FOUNDEROS_MIGRATION_PROMPT ??= "never";
  env.FOUNDEROS_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[founderos] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.FOUNDEROS_BIND = effectiveBind;
  if (bindHost) {
    env.FOUNDEROS_BIND_HOST = bindHost;
  } else {
    delete env.FOUNDEROS_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.FOUNDEROS_DEPLOYMENT_MODE;
    delete env.FOUNDEROS_DEPLOYMENT_EXPOSURE;
    delete env.FOUNDEROS_AUTH_BASE_URL_MODE;
    console.log("[founderos] dev mode: local_trusted (bind=loopback)");
  } else {
    env.FOUNDEROS_DEPLOYMENT_MODE = "authenticated";
    env.FOUNDEROS_DEPLOYMENT_EXPOSURE = "private";
    env.FOUNDEROS_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[founderos] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.FOUNDEROS_BIND;
  delete env.FOUNDEROS_BIND_HOST;
  delete env.FOUNDEROS_DEPLOYMENT_MODE;
  delete env.FOUNDEROS_DEPLOYMENT_EXPOSURE;
  delete env.FOUNDEROS_AUTH_BASE_URL_MODE;
  console.log("[founderos] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[founderos] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@founderos/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[founderos] Command failed with code ${status.code}: pnpm --filter @founderos/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[founderos] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.FOUNDEROS_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[founderos] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[founderos] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@founderos/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[founderos] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@founderos/server", serverScript, ...forwardedArgs],
