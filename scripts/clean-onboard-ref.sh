...
Environment overrides:
  ...
  PC_DATA=/tmp/data           FounderOS data dir to use
  FOUNDEROS_HOST=127.0.0.1    Host passed to the onboarded server
  FOUNDEROS_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/founderos-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/founderos-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export FOUNDEROS_HOST="${FOUNDEROS_HOST:-127.0.0.1}"
export FOUNDEROS_PORT="${FOUNDEROS_PORT:-3100}"
export FOUNDEROS_OPEN_ON_LISTEN="${FOUNDEROS_OPEN_ON_LISTEN:-false}"
...
echo "FOUNDEROS_HOST: $FOUNDEROS_HOST"
echo "FOUNDEROS_PORT: $FOUNDEROS_PORT"
...
env \
  ...
  HOST="$FOUNDEROS_HOST" \
  PORT="$FOUNDEROS_PORT" \
  FOUNDEROS_OPEN_ON_LISTEN="$FOUNDEROS_OPEN_ON_LISTEN" \
  pnpm founderos onboard --yes --data-dir "$PC_DATA"
