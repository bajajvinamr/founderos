...
const DEFAULT_QUERY = "\"Co-Authored-By: FounderOS <noreply@founderos.ai>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/founderos-commit-metrics-cache.json");
...
const FOUNDEROS_EMAIL = "noreply@founderos.ai";
const FOUNDEROS_NAME = "founderos";
...
function printHelp() {
  console.log(`Usage: tsx scripts/founderos-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === FOUNDEROS_EMAIL) || (displayName && displayName.toLowerCase() === FOUNDEROS_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("FounderOS commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "founderos-commit-metrics",
