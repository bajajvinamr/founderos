...
FOUNDEROS_API_URL="${FOUNDEROS_API_URL:-http://127.0.0.1:3100}"
API_BASE="${FOUNDEROS_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-founderos-smoke}"
...
FOUNDEROS_API_URL_FOR_OPENCLAW="${FOUNDEROS_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${FOUNDEROS_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${FOUNDEROS_AUTH_HEADER}" )
fi
if [[ -n "${FOUNDEROS_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${FOUNDEROS_COOKIE}" )
  FOUNDEROS_BROWSER_ORIGIN="${FOUNDEROS_BROWSER_ORIGIN:-${FOUNDEROS_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${FOUNDEROS_BROWSER_ORIGIN}" -H "Referer: ${FOUNDEROS_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set FOUNDEROS_COOKIE or FOUNDEROS_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg founderosApiUrl "$FOUNDEROS_API_URL_FOR_OPENCLAW" \
    ...
    '{
      ...
      agentDefaultsPayload: {
        ...
        sessionKey: "founderos",
        timeoutSec: $timeoutSec,
        waitTimeoutMs: $waitTimeoutMs,
        founderosApiUrl: $founderosApiUrl
      }
    }')"
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/founderos"
  local claimed_file="${workspace_dir}/founderos-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/founderos-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/founderos-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/founderos-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/founderos"
    assert_status "200"
    {
      echo "FOUNDEROS_API_URL=${FOUNDEROS_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set FOUNDEROS_API_KEY=${AGENT_API_KEY} in your run context before running FounderOS heartbeat steps."
...
main() {
  ...
  wait_http_ready "${FOUNDEROS_API_URL%/}/api/health" 15 || fail "FounderOS API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "founderos health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
