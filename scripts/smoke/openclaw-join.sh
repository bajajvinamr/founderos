...
COMPANY_ID="${COMPANY_ID:-${FOUNDEROS_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-founderos-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-founderos-openclaw-smoke}"
...
Provide one of:
  FOUNDEROS_AUTH_HEADER=\"Bearer <board-token>\"
  FOUNDEROS_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "FounderOS OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.founderos.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
