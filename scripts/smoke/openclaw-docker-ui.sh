...
FOUNDEROS_HOST_PORT="${FOUNDEROS_HOST_PORT:-3100}"
FOUNDEROS_HOST_FROM_CONTAINER="${FOUNDEROS_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.founderos-openclaw.override.yml"
...
detect_founderos_base_url() {
  ...
  for candidate in "$FOUNDEROS_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${FOUNDEROS_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${FOUNDEROS_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
founderos_base_url="$(detect_founderos_base_url || true)"
...
FounderOS URL for OpenClaw container:
EOF
  if [[ -n "$founderos_base_url" ]]; then
    cat <<EOF
  $founderos_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${FOUNDEROS_HOST_PORT}
  (Do not use http://127.0.0.1:${FOUNDEROS_HOST_PORT} inside the container.)
  If FounderOS rejects the host, run on host machine:
    pnpm founderos allowed-hostname host.docker.internal
  Then restart FounderOS and re-run this script.
EOF
  fi
  cat <<EOF
