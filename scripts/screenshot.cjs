#!/usr/bin/env node
/**
 * Screenshot utility for FounderOS UI.
 *
 * Reads the board token from ~/.founderos/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/founderos-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".founderos/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.founderos/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the FounderOS origin only
