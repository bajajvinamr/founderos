...
- [~] Audit trail visible to founder — DB layer closed for the 2026-05-07 cycle (magic-link issue/consume PR-6a, agent pause/resume/terminate/delete PR-6b regression-pinned, billing-gate 402 PR-6c). UI surfacing of these new `action` values in the activity timeline component is a separate piece of work — covered by Day 4 audit-completeness stream in the dream-state directive.
...
- [~] **Pause cascades — paused company halts all its agents** — partial. `paused-agent-blocks-heartbeat.test.ts` covers the agent-level guard; `department-status.test.ts` covers department-level pause. Company-level pause cascade (one PAUSE → all agents under that companyId are gated) is not pinned by a single test today; the underlying mechanism is the same per-agent guard fired N times. Tighter coverage queued as a follow-up regression test.
