...
- [x] **Background job (cron) failure is logged with structured fields + alerts** — closed by PR #52 (commit `06f442c`, merged 2026-05-07). New `runCronTick(taskName, fn)` helper at `server/src/lib/cron-tick.ts` composes `runInCronContext` + try/catch + `captureServerError` + structured duration logging. 6 setInterval-based schedulers migrated (daily-digest, decision-followup, linkedin-sync, notion-sync, slack-sync, weekly-wrap-delivery); the 7th (hubspot-sync) is BullMQ Worker shape and is queued as PR-5b. 10 helper tests + 4 existing cron-touching suites all green (54/54 in 3.0s). Sentry capture is a no-op until `SENTRY_DSN` is set, so the wiring lands cleanly ahead of secret rotation.
...
- [ ] **PR-5b** — Cron observability for `hubspot-sync-cron` (BullMQ Worker shape, not setInterval). Wrap worker callback in `runInCronContext`; route `worker.on("failed")` through `captureServerError`. Smallest-blast-radius next item; queued for Cycle 4.
