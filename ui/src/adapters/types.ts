...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@founderos/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@founderos/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@founderos/adapter-utils").TranscriptEntry[];
