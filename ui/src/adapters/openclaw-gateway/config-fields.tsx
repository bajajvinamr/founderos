...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="FounderOS API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "founderosApiUrl",
                  String(config.founderosApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "founderosApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://founderos.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/founderos-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "founderos"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="founderos"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. FounderOS persists a device key during onboarding so pairing approvals
