...
/**
 * Filter .founderos.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterFounderOSYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm founderos company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [FounderOS](https://founderos.ai) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [FounderOS](https://founderos.ai) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .founderos.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .founderos.yaml
    const yamlPath = exportData.founderosExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterFounderOSYaml(exportData.files[yamlPath], checkedFiles);
    }
