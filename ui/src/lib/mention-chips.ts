...
import { parseAgentMentionHref, parseProjectMentionHref, parseSkillMentionHref } from "@founderos/shared";
...
export function mentionChipInlineStyle(mention: ParsedMentionChip): CSSProperties | undefined {
  ...
  if (mention.kind === "project" && mention.color) {
    ...
    style["--founderos-mention-project-color"] = mention.color;
  }

  if (mention.kind === "agent") {
    const iconMask = buildAgentIconMask(mention.icon);
    if (iconMask) {
      style["--founderos-mention-icon-mask"] = iconMask;
    }
  }
...
export function applyMentionChipDecoration(element: HTMLElement, mention: ParsedMentionChip) {
  ...
  element.classList.add("founderos-mention-chip", `founderos-mention-chip--${mention.kind}`);
  if (mention.kind === "project") {
    element.classList.add("founderos-project-mention-chip");
  }
...
export function clearMentionChipDecoration(element: HTMLElement) {
  ...
  element.classList.remove(
    "founderos-mention-chip",
    "founderos-mention-chip--agent",
    "founderos-mention-chip--project",
    "founderos-mention-chip--skill",
    "founderos-project-mention-chip",
  );
  element.removeAttribute("contenteditable");
  element.style.removeProperty("border-color");
  element.style.removeProperty("background-color");
  element.style.removeProperty("color");
  element.style.removeProperty("--founderos-mention-project-color");
  element.style.removeProperty("--founderos-mention-icon-mask");
}
