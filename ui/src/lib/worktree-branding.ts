...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("founderos-worktree-enabled") !== "true") return null;

  const name = readMetaContent("founderos-worktree-name");
  const color = normalizeHexColor(readMetaContent("founderos-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("founderos-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
