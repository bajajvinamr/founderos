...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`founderos-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="founderos-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("founderos-mermaid-status", error && "founderos-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="founderos-mermaid-source">
...
export function MarkdownBody({
        ...
        return (
          <a
            ...
            className={cn(
              ...
              `founderos-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "founderos-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "founderos-markdown prose prose-sm max-w-none break-words overflow-hidden",
