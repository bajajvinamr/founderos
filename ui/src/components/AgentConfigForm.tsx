...
} from "@founderos/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@founderos/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@founderos/adapter-gemini-local";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
// Canonical type lives in @founderos/adapter-utils; re-exported here
// so existing imports from this file keep working.
export type { CreateConfigValues } from "@founderos/adapter-utils";
import type { CreateConfigValues } from "@founderos/adapter-utils";
  ...
  return (
    <div className={cn("relative", cards && "space-y-6")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label="Capabilities" hint={help.capabilities}>
              <MarkdownEditor
                ...
                placeholder="Describe what this teammate can do..."
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {!isCreate && (
                <>
                  <Field label="Shift timeout (sec)" hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> Shift schedule</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> Shift schedule</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              ...
              numberLabel="sec"
              numberPrefix="Start a shift every"
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              ...
              />
              <Field label="Rest between shifts (sec)" hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label="Max parallel shifts" hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
