...
type FounderOSSprite = (typeof FOUNDEROS_SPRITES)[number];
...
function spriteSize(sprite: FounderOSSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = FOUNDEROS_SPRITES[(baseCol + baseRow) % FOUNDEROS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = FOUNDEROS_SPRITES[Math.floor(Math.random() * FOUNDEROS_SPRITES.length)]!;
