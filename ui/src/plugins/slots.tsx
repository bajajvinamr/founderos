 ...
 *
 * Plugin bundles are built with `external: ["@founderos/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__founderosPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__founderosPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__founderosPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__founderosPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@founderos/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@founderos/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@founderos/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@founderos/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@founderos/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__founderosPluginBridge__) {
 ...
 * self-register via `window.founderosPlugins.registerReactComponent()`.
 ...
 * of `@founderos/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
