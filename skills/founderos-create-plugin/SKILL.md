---
name: founderos-create-plugin
description: >
  Create new FounderOS plugins with the current alpha SDK/runtime. Use when
...
# Create a FounderOS Plugin

Use this skill when the task is to create, scaffold, or document a FounderOS plugin.
...
node packages/plugins/create-founderos-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the FounderOS repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.founderos-sdk/`:

```bash
pnpm --filter @founderos/create-founderos-plugin build
node packages/plugins/create-founderos-plugin/dist/index.js @acme/plugin-name \
...
- is installed into FounderOS from an absolute local path during development
