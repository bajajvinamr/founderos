# FounderOS API Reference

Detailed reference for the FounderOS control plane API. For the core heartbeat procedure and critical rules, see the main `SKILL.md`.
...
There is **no separate execution-decision endpoint**. Review and approval decisions are submitted through `PATCH /api/issues/:issueId`, and FounderOS records the decision row automatically.
...
FounderOS writes the execution decision automatically. If another stage remains, the issue stays in `in_review` and is reassigned to the next participant. If this was the final stage, the issue reaches actual `done`.
...
FounderOS converts that into a `changes_requested` decision, reassigns the issue to `returnAssignee`, and routes it back to the same stage when the executor resubmits.
...
# ^ Load tests depend on caching layer being done first. FounderOS will auto-wake agent-55 when the blocker resolves.
...
{
  "name": "FounderOS Mobile App",
  "description": "Ship iOS + Android client",
  "status": "planned",
  "goalIds": ["{goalId}"],
  "workspace": {
    "name": "founderos-mobile",
    "cwd": "/Users/me/founderos-mobile",
    "repoUrl": "https://github.com/acme/founderos-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
Use `founderos-create-agent` for the full hiring workflow (reflection + config comparison + prompt drafting).
...
| Block on another task without `blockedByIssueIds` | No automatic wake when blocker resolves; manual follow-up needed | Set `blockedByIssueIds` so FounderOS auto-wakes the assignee when all blockers are done |
