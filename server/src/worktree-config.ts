...
import type { FounderOSConfig } from "@founderos/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by FounderOS worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.FOUNDEROS_IN_WORKTREE !== "true") return null;

  const configPath = resolveFounderOSConfigPath(overrideConfigPath);
  const envPath = resolveFounderOSEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.FOUNDEROS_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.FOUNDEROS_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.FOUNDEROS_HOME) ??
      nonEmpty(env.FOUNDEROS_WORKTREES_DIR) ??
      "~/.founderos-worktrees",
  );
...
function writeConfigFile(configPath: string, config: FounderOSConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".founderos", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as FounderOSConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: FounderOSConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: FounderOSConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.FOUNDEROS_HOME = context.homeDir;
  process.env.FOUNDEROS_INSTANCE_ID = context.instanceId;
  process.env.FOUNDEROS_CONFIG = context.configPath;
  process.env.FOUNDEROS_CONTEXT = context.contextPath;
  process.env.FOUNDEROS_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as FounderOSConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    FOUNDEROS_HOME: context.homeDir,
    FOUNDEROS_INSTANCE_ID: context.instanceId,
    FOUNDEROS_CONFIG: context.configPath,
    FOUNDEROS_CONTEXT: context.contextPath,
    FOUNDEROS_IN_WORKTREE: "true",
    FOUNDEROS_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: FounderOSConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as FounderOSConfig;
  } catch {
    return;
  }
