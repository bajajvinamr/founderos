...
import { issues, projects, projectWorkspaces } from "@founderos/db";
import { updateExecutionWorkspaceSchema } from "@founderos/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  router.post("/execution-workspaces/:id/runtime-services/:action", async (req, res) => {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before FounderOS can manage local runtime services" });
      return;
    }
