...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    ...
    normalized !== "founderos-create-agent" &&
    normalized !== "founderos-create-plugin" &&
...
/** Resolve the FounderOS repo skills directory (built-in / managed skills). */
function resolveFounderOSSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const founderosSkillsDir = resolveFounderOSSkillsDir();

  // Build set of FounderOS-managed skill names
  const founderosSkillNames = new Set<string>();
  if (founderosSkillsDir) {
    try {
      for (const entry of fs.readdirSync(founderosSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) founderosSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isFounderOSManaged: founderosSkillNames.has(entry.name),
      });
    }
...
function normalizeHeaderMap(
  ...
  for (const [key, value] of entries) {
    ...
    if (PROTO_KEYS.has(trimmedKey)) continue;
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.founderosApiUrl)) {
    const legacyFounderOSApiUrl = nonEmptyTrimmedString(input.founderosApiUrl);
    if (legacyFounderOSApiUrl) merged.founderosApiUrl = legacyFounderOSApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    founderosApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.founderosApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawFounderOSApiUrl =
    typeof defaults.founderosApiUrl === "string"
      ? defaults.founderosApiUrl.trim()
      : "";
  if (rawFounderOSApiUrl) {
    try {
      const parsedFounderOSApiUrl = new URL(rawFounderOSApiUrl);
      if (
        parsedFounderOSApiUrl.protocol !== "http:" &&
        parsedFounderOSApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_founderos_api_url_protocol",
          level: "warn",
          message: `founderosApiUrl must use http:// or https:// (got ${parsedFounderOSApiUrl.protocol}).`
        });
      } else {
        normalized.founderosApiUrl = parsedFounderOSApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_founderos_api_url_configured",
          level: "info",
          message: `founderosApiUrl set to ${parsedFounderOSApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_founderos_api_url_invalid",
        level: "warn",
        message: `Invalid founderosApiUrl: ${rawFounderOSApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your FounderOS host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "FounderOS is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm founderos allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/founderos";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/founderos-claimed-api-key.json and load FOUNDEROS_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: founderosApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the FounderOS hostname is reachable and allowed via `pnpm founderos allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this FounderOS API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/founderos/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # FounderOS OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          founderosApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, FounderOS generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "founderosApiUrl": "https://founderos-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in FounderOS before key claim is allowed.
    ...
    - ~/.openclaw/workspace/founderos-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/founderos-claimed-api-key.json

    And set the FOUNDEROS_API_KEY and FOUNDEROS_API_URL in your environment variables as specified here:
    ...
    Then set FOUNDEROS_API_KEY and FOUNDEROS_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install FounderOS skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your FOUNDEROS_API_URL to the top of your skill and note the path to your FOUNDEROS_API_URL
    ...
    ${
      ...
      "Ensure FounderOS is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested FounderOS base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.founderosApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm founderos allowed-hostname <host>
      - then restart FounderOS and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@founderos.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "founderos", path: "/api/skills/founderos" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "founderos-create-agent",
          path: "/api/skills/founderos-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              founderosApiUrl: req.body.founderosApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.founderosApiUrl &&
          !persistedDefaults.founderosApiUrl
        ) {
          missingPersistedFields.push("founderosApiUrl");
        }
