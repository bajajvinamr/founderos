...
import { issueExecutionDecisions } from "@founderos/db";
import {
...
import { trackAgentTaskCompleted } from "@founderos/shared/telemetry";
...
import { registerDocumentRoutes } from "./issues-documents.js";
import { registerExecutionRoutes, type ExecutionRouteDeps } from "./issues-execution.js";
...
export function issueRoutes(
  ...
  function respondClosedIssueExecutionWorkspace(
    res: Response,
    workspace: { closedAt: Date | null; id: string; mode: string; name: string | null; status: string },
  ) {
    res.status(409).json({
      error: getClosedIsolatedExecutionWorkspaceMessage(workspace as { name: string }),
