...
import { runClaudeLogin } from "@founderos/adapter-claude-local/server";
import {
...
import { ensureOpenCodeModelConfiguredAndAvailable } from "@founderos/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.FOUNDEROS_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeFounderOSSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readFounderOSSkillSyncPreference(
  ...
  router.post("/agents/:id/wakeup", agentInvokeLimiter, billingGate(db), validate(wakeAgentSchema), async (req, res) => {
    const id = req.params.id as string;
    const agent = await svc.getById(id);
    if (!agent) {
      res.status(404).json({ error: "Agent not found" });
  ...
  router.post("/agents/:id/heartbeat/invoke", agentInvokeLimiter, billingGate(db), async (req, res) => {
    const id = req.params.id as string;
    const agent = await svc.getById(id);
    if (!agent) {
      res.status(404).json({ error: "Agent not found" });
