...
import type { CompanyPortabilityFileEntry } from "@founderos/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "founderos",
        name: "founderos",
        description: "FounderOS coordination skill",
        markdown: "---\nname: founderos\ndescription: FounderOS coordination skill\n---\n\n# FounderOS\n",
        sourceType: "github",
        sourceLocator: "https://github.com/founderos-ai/founderos/tree/master/skills/founderos",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/founderos/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "founderos",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/founderos/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "founderos",
  ...
  it("exports referenced skills as stubs by default with sanitized FounderOS extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "FounderOS"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${founderosKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/founderos-ai/founderos/founderos/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/founderos-ai/founderos/founderos/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/founderos-ai/founderos/founderos/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".founderos.yaml"]);
    expect(extension).toContain('schema: "founderos/v1"');
    ...
    expect(extension).not.toContain("founderosSkillSync");
  ...
  it("exports default sidebar order into the FounderOS extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".founderos.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/founderos-ai/founderos/founderos/SKILL.md"])).toContain("# FounderOS");
    expect(asTextFile(exported.files["skills/founderos-ai/founderos/founderos/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/founderos-ai/founderos/founderos/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/founderos-ai/founderos/founderos/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/founderos-ai/founderos/founderos/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .founderos.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".founderos.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-founderos",
        companyId: "company-1",
        key: "founderos-ai/founderos/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/founderos-ai/founderos/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "founderos_bundled",
          owner: "founderos",
          repo: "founderos",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/founderos-ai/founderos/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/founderos-ai/founderos/release-changelog/SKILL.md"])).toContain("founderos-ai/founderos/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/founderos",
            repoUrl: "https://github.com/founderos-ai/founderos.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .founderos-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/founderos-ai/founderos.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/founderos");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported FounderOS",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/founderos-ai/founderos.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".founderos.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/founderos-ai/founderos.git"');
    expect(extension).toContain('projectWorkspaceKey: "founderos"');
  ...
  it("reads env inputs back from .founderos.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .founderos.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with FounderOS routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "founderos-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported FounderOS" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported FounderOS"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .founderos.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported FounderOS");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".founderos.yaml"] = `${exported.files[".founderos.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
