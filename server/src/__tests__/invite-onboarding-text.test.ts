...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("FounderOS OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested FounderOS base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("founderosApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.founderosApiUrl");
    expect(text).toContain("~/.openclaw/workspace/founderos-claimed-api-key.json");
    expect(text).toContain("FOUNDEROS_API_KEY");
