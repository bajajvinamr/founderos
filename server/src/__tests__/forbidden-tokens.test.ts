...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "founderos", LOGNAME: "founderos", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "founderos" }),
      },
    );

    expect(tokens).toEqual(["founderos", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "founderos" }, {
        userInfo: () => ({ username: "founderos" }),
      });

      expect(tokens).toEqual(["founderos", "custom-token"]);
    } finally {
