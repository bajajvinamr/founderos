...
describe("agent instructions service", () => {
  const originalFounderOSHome = process.env.FOUNDEROS_HOME;
  const originalFounderOSInstanceId = process.env.FOUNDEROS_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalFounderOSHome === undefined) delete process.env.FOUNDEROS_HOME;
    else process.env.FOUNDEROS_HOME = originalFounderOSHome;
    if (originalFounderOSInstanceId === undefined) delete process.env.FOUNDEROS_INSTANCE_ID;
    else process.env.FOUNDEROS_INSTANCE_ID = originalFounderOSInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const founderosHome = await makeTempDir("founderos-agent-instructions-home-");
    const externalRoot = await makeTempDir("founderos-agent-instructions-external-");
    cleanupDirs.add(founderosHome);
    cleanupDirs.add(externalRoot);
    process.env.FOUNDEROS_HOME = founderosHome;
    process.env.FOUNDEROS_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("founderos-agent-instructions-new-external-");
    cleanupDirs.add(founderosHome);
    cleanupDirs.add(externalRoot);
    process.env.FOUNDEROS_HOME = founderosHome;
    process.env.FOUNDEROS_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("founderos-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const founderosHome = await makeTempDir("founderos-agent-instructions-recover-");
    cleanupDirs.add(founderosHome);
    process.env.FOUNDEROS_HOME = founderosHome;
    process.env.FOUNDEROS_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const founderosHome = await makeTempDir("founderos-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("founderos-agent-instructions-stale-root-");
    cleanupDirs.add(founderosHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const founderosHome = await makeTempDir("founderos-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("founderos-agent-instructions-heal-write-stale-");
    cleanupDirs.add(founderosHome);
    cleanupDirs.add(staleRoot);
    process.env.FOUNDEROS_HOME = founderosHome;
    process.env.FOUNDEROS_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const founderosHome = await makeTempDir("founderos-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("founderos-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(founderosHome);
    cleanupDirs.add(staleRoot);
    process.env.FOUNDEROS_HOME = founderosHome;
    process.env.FOUNDEROS_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const founderosHome = await makeTempDir("founderos-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("founderos-agent-instructions-partial-root-");
    cleanupDirs.add(founderosHome);
    cleanupDirs.add(staleRoot);
    process.env.FOUNDEROS_HOME = founderosHome;
    process.env.FOUNDEROS_INSTANCE_ID = "test-instance";
