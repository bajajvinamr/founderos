...
function registerModuleMocks() {
  vi.doMock("@founderos/shared/telemetry", () => ({
...
describe("agent skill routes", () => {
  beforeEach(() => {
    ...
    mockCompanySkillService.resolveRequestedSkillKeys.mockImplementation(
      async (_companyId: string, requested: string[]) =>
        requested.map((value) =>
          value === "founderos"
            ? "founderos-ai/founderos/founderos"
            : value,
        ),
    );
    ...
    expect(mockAdapter.listSkills).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        config: expect.objectContaining({
          founderosRuntimeSkills: expect.any(Array),
        }),
      }),
    );
  }, 10_000);
  ...
  it("skips runtime materialization when syncing Claude skills", async () => {
    ...
    const res = await request(await createApp())
      ...
      .send({ desiredSkills: ["founderos-ai/founderos/founderos"] });
    ...
    expect(mockAgentService.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          founderosSkillSync: expect.objectContaining({
            desiredSkills: ["founderos-ai/founderos/founderos"],
          }),
        }),
      }),
