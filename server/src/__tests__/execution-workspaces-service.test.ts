...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "FounderOS Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@founderos.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("founderos-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `founderos-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "founderos-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "founderos-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "founderos-close-check",
      baseRef: "main",
      metadata: {
  ...
  it("shows inherited shared project runtime services on shared execution workspaces without duplicating old history", async () => {
    ...
    const reuseKey = `project_workspace:${projectWorkspaceId}:founderos-dev`;
    ...
    await db.insert(projectWorkspaces).values({
      ...
      metadata: {
        runtimeConfig: {
          ...
          workspaceRuntime: {
            services: [{ name: "founderos-dev", command: "pnpm dev" }],
          },
        },
      },
    });
