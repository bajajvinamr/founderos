...
describe("pi local skill sync", () => {
  ...
  it("reports configured FounderOS skills and installs them into the Pi skills home", async () => {
    const home = await makeTempDir("founderos-pi-skill-sync-");
    ...
    const after = await syncPiSkills(ctx, [founderosKey]);
    expect(after.entries.find((entry) => entry.key === founderosKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".pi", "agent", "skills", "founderos"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled FounderOS skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("founderos-pi-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncPiSkills(configuredCtx, [founderosKey]);
