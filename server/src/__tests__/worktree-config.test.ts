...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const founderosDir = path.join(worktreeRoot, ".founderos");
    const configPath = path.join(founderosDir, "config.json");
    const envPath = path.join(founderosDir, ".env");
    const sharedRoot = path.join(tempRoot, ".founderos", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".founderos-worktrees");

    await fs.mkdir(founderosDir, { recursive: true });
    ...
    await fs.writeFile(
      ...
      [
        ...
        "FOUNDEROS_WORKTREE_NAME=PAP-884-ai-commits-component",
        "FOUNDEROS_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.FOUNDEROS_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.FOUNDEROS_WORKTREES_DIR = isolatedHome;
    delete process.env.FOUNDEROS_HOME;
    delete process.env.FOUNDEROS_INSTANCE_ID;
    delete process.env.FOUNDEROS_CONFIG;
    delete process.env.FOUNDEROS_CONTEXT;
    ...
    expect(repairedEnv).toContain(`FOUNDEROS_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('FOUNDEROS_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`FOUNDEROS_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`FOUNDEROS_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('FOUNDEROS_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.FOUNDEROS_HOME).toBe(isolatedHome);
    expect(process.env.FOUNDEROS_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# FounderOS environment variables",
        "FOUNDEROS_IN_WORKTREE=true",
        "FOUNDEROS_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.FOUNDEROS_IN_WORKTREE = "true";
    process.env.FOUNDEROS_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.FOUNDEROS_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".founderos-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".founderos", "worktrees");
    ...
    const founderosDir = path.join(currentWorktreeRoot, ".founderos");
    const configPath = path.join(founderosDir, "config.json");
    const envPath = path.join(founderosDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".founderos", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.FOUNDEROS_IN_WORKTREE = "true";
    process.env.FOUNDEROS_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.FOUNDEROS_HOME = isolatedHome;
    process.env.FOUNDEROS_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.FOUNDEROS_CONFIG = configPath;
