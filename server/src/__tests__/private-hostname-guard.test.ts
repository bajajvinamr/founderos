...
describe("privateHostnameGuard", () => {
  ...
  it("blocks unknown hostnames with remediation command", async () => {
    ...
    expect(res.body?.error).toContain(`please run pnpm founderos allowed-hostname ${unknownHostname}`);
  });
  ...
  it("blocks unknown hostnames on page routes with plain-text remediation command", async () => {
    ...
    expect(res.status).toBe(403);
    expect(res.text).toContain(`please run pnpm founderos allowed-hostname ${unknownHostname}`);
  }, 20_000);
});
