...
describe("feedback trace share client", () => {
  ...
  it("defaults to telemetry.founderos.ai when no backend url is configured", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://telemetry.founderos.ai/feedback-traces",
  ...
  it("wraps the feedback trace payload as gzip+base64 json before upload", async () => {
    const client = createFeedbackTraceShareClientFromConfig({
      feedbackExportBackendUrl: "https://telemetry.founderos.ai",
    ...
    expect(call?.[0]).toBe("https://telemetry.founderos.ai/feedback-traces");
