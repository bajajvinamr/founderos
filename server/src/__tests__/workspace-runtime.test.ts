...
import { resolveFounderOSConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@founderos/shared";
...
async function createTempRepo(defaultBranch = "main") {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "founderos@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "FounderOS Test"]);
...
afterEach(async () => {
  ...
  delete process.env.FOUNDEROS_CONFIG;
  delete process.env.FOUNDEROS_HOME;
  delete process.env.FOUNDEROS_INSTANCE_ID;
  delete process.env.FOUNDEROS_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited FounderOS and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/founderos",
      FOUNDEROS_HOME: "/tmp/founderos-home",
      FOUNDEROS_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.FOUNDEROS_HOME).toBeUndefined();
    expect(sanitized.FOUNDEROS_INSTANCE_ID).toBeUndefined();
...
describe("ensureServerWorkspaceLinksCurrent", () => {
  it("relinks stale server workspace dependencies inside the current repo root", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-links-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-links-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@founderos");
    ...
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/founderos-main/.git/worktrees/runtime-links\n", "utf8");
    await fs.writeFile(path.join(repoRoot, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n  - server\n", "utf8");
    await fs.writeFile(
      path.join(repoRoot, "server", "package.json"),
      JSON.stringify({
        name: "@founderos/server",
        dependencies: {
          "@founderos/db": "workspace:*",
        },
      }),
      "utf8",
    );
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@founderos/db" }),
      "utf8",
    );
    await fs.writeFile(
  ...
  it("skips relinking when server workspace dependencies already point at the repo", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-links-current-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@founderos");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    ...
    await fs.mkdir(expectedPackageDir, { recursive: true });
    await fs.mkdir(serverNodeModulesScopeDir, { recursive: true });
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/founderos-main/.git/worktrees/runtime-links-current\n", "utf8");
    await fs.writeFile(path.join(repoRoot, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n  - server\n", "utf8");
    await fs.writeFile(
      path.join(repoRoot, "server", "package.json"),
      JSON.stringify({
  ...
  it("skips relinking outside linked git worktrees", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-links-non-worktree-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-links-non-worktree-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@founderos");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    const stalePackageDir = path.join(staleRoot, "db");
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".founderos", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$FOUNDEROS_WORKSPACE_BRANCH\" > .founderos-provision-branch",
        "printf '%s\\n' \"$FOUNDEROS_WORKSPACE_BASE_CWD\" > .founderos-provision-base",
        "printf '%s\\n' \"$FOUNDEROS_WORKSPACE_CREATED\" > .founderos-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".founderos-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".founderos-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".founderos-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".founderos-provision-created"), "utf8")).resolves.toBe("false\n");
  });
  ...
  it("uses the latest repo-managed provision script when reusing an existing worktree", async () => {
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".founderos-provision-version"), "utf8")).resolves.toBe("v1\n");
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".founderos-provision-version"), "utf8")).resolves.toBe("v2\n");
  });

  it("writes an isolated repo-local FounderOS config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const founderosHome = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(founderosHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.FOUNDEROS_HOME = founderosHome;
    process.env.FOUNDEROS_INSTANCE_ID = instanceId;
    process.env.FOUNDEROS_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/founderos"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".founderos", "config.json");
      const envPath = path.join(workspace.cwd, ".founderos", ".env");
      ...
      expect(envVars.FOUNDEROS_HOME).toBe(isolatedWorktreeHome);
      expect(envVars.FOUNDEROS_INSTANCE_ID).toBe(expectedInstanceId);
      expect(await fs.realpath(envVars.FOUNDEROS_CONFIG!)).toBe(await fs.realpath(configPath));
      expect(envVars.FOUNDEROS_IN_WORKTREE).toBe("true");
      expect(envVars.FOUNDEROS_WORKTREE_NAME).toBe("PAP-885-show-worktree-banner");

      process.chdir(workspace.cwd);
      expect(resolveFounderOSConfigPath()).toBe(configPath);
    } finally {
  ...
  it("provisions successfully when install is needed but there are no symlinked node_modules to move", async () => {
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".founderos", "config.json"), "utf8")).resolves.toContain(
      "\"database\"",
    );
  }, 30_000);
  ...
  it("auto-detects the default branch when baseRef is not configured", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-worktree-bare-"));
  ...
  it("auto-detects the default branch via symbolic-ref when origin/HEAD is set", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-worktree-bare-symref-"));
    await runGit(bareRemote, ["init", "--bare"]);
    await runGit(repoRoot, ["remote", "add", "origin", bareRemote]);
    await runGit(repoRoot, ["push", "-u", "origin", "master"]);
    await runGit(repoRoot, ["fetch", "origin"]);
...
describe("ensureRuntimeServicesForRun", () => {
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".founderos", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.FOUNDEROS_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              FOUNDEROS_HOME: "{{workspace.cwd}}/.founderos/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".founderos", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".founderos", "runtime-services"));
  });

  it("does not leak parent FounderOS instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "founderosConfig: process.env.FOUNDEROS_CONFIG ?? null,",
          "founderosHome: process.env.FOUNDEROS_HOME ?? null,",
          "founderosInstanceId: process.env.FOUNDEROS_INSTANCE_ID ?? null,",
    ...
    process.env.FOUNDEROS_CONFIG = "/tmp/base-founderos-config.json";
    process.env.FOUNDEROS_HOME = "/tmp/base-founderos-home";
    process.env.FOUNDEROS_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/founderos";
    ...
    expect(captured.founderosConfig).toBeNull();
    expect(captured.founderosHome).toBeNull();
    expect(captured.founderosInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-sibling-"));
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("founderos-workspace-runtime-");
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-reconcile-"));
    const founderosHome = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-home-"));
    process.env.FOUNDEROS_HOME = founderosHome;
    process.env.FOUNDEROS_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("marks persisted local services stopped when the registry pid is stale", async () => {
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      serviceName: "founderos-dev",
      status: "running",
      lifecycle: "shared",
      reuseKey: `project_workspace:${projectWorkspaceId}:founderos-dev`,
      command: "pnpm dev",
      cwd: "/tmp/founderos-primary",
      port: 49195,
    ...
    await writeLocalServiceRegistryRecord({
      version: 1,
      serviceKey: "workspace-runtime-founderos-dev-stale",
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "founderos-runtime-stop-persisted-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const runId = randomUUID();
