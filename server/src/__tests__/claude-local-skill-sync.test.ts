...
describe("claude local skill sync", () => {
  ...
  const createAgentKey = "founderos-ai/founderos/founderos-create-agent";
  ...
  it("defaults to mounting all built-in FounderOS skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === founderosKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat FounderOS skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("founderos-claude-user-skills-");
