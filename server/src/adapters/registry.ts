...
import { getAdapterSessionManagement } from "@founderos/adapter-utils";
import {
...
} from "@founderos/adapter-claude-local/server";
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@founderos/adapter-claude-local";
import {
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@founderos/adapter-codex-local";
import {
...
} from "@founderos/adapter-cursor-local/server";
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@founderos/adapter-cursor-local";
import {
...
} from "@founderos/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@founderos/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@founderos/adapter-openclaw-gateway";
...
import {
  ...
  listPiModels,
} from "@founderos/adapter-pi-local/server";
import {
  agentConfigurationDoc as piAgentConfigurationDoc,
} from "@founderos/adapter-pi-local";
import {
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[founderos] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[founderos] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[founderos] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[founderos] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
