...
function extractUiParserSource(
  ...
  const contractVersion = pkg.founderos?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no founderos.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
