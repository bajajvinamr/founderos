const FAVICON_BLOCK_START = "<!-- FOUNDEROS_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- FOUNDEROS_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- FOUNDEROS_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- FOUNDEROS_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.FOUNDEROS_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.FOUNDEROS_WORKTREE_NAME) ?? nonEmpty(env.FOUNDEROS_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.FOUNDEROS_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="founderos-worktree-enabled" content="true" />',
    `<meta name="founderos-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="founderos-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="founderos-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
