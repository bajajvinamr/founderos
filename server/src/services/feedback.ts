...
import { readFounderOSSkillSyncPreference } from "@founderos/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@founderos/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@founderos/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@founderos/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveFounderOSInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "founderos-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "founderos-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "founderos-feedback-v1";
const FEEDBACK_DESTINATION = "founderos_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "founderos_run" &&
    file.source !== "founderos_run_events" &&
    file.source !== "founderos_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.FOUNDEROS_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readFounderOSSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let founderosRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      founderosRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "founderos/run.json",
        contentType: "application/json",
        source: "founderos_run",
        contents: `${JSON.stringify(founderosRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.founderosRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "founderos/run-events.json",
        contentType: "application/json",
        source: "founderos_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "founderos/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "founderos_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.founderosRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
