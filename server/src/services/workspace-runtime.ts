...
import type { AdapterRuntimeServiceReport } from "@founderos/adapter-utils";
import type { Db } from "@founderos/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@founderos/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.FOUNDEROS_WORKSPACE_CWD = input.worktreePath;
  env.FOUNDEROS_WORKSPACE_PATH = input.worktreePath;
  env.FOUNDEROS_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.FOUNDEROS_WORKSPACE_BRANCH = input.branchName;
  env.FOUNDEROS_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.FOUNDEROS_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.FOUNDEROS_WORKSPACE_SOURCE = input.base.source;
  env.FOUNDEROS_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.FOUNDEROS_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.FOUNDEROS_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.FOUNDEROS_PROJECT_ID = input.base.projectId ?? "";
  env.FOUNDEROS_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.FOUNDEROS_AGENT_ID = input.agent.id ?? "";
  env.FOUNDEROS_AGENT_NAME = input.agent.name;
  env.FOUNDEROS_COMPANY_ID = input.agent.companyId;
  env.FOUNDEROS_ISSUE_ID = input.issue?.id ?? "";
  env.FOUNDEROS_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.FOUNDEROS_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.FOUNDEROS_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.FOUNDEROS_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.FOUNDEROS_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.FOUNDEROS_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.FOUNDEROS_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.FOUNDEROS_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.FOUNDEROS_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.FOUNDEROS_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.FOUNDEROS_PROJECT_ID = input.workspace.projectId ?? "";
  env.FOUNDEROS_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.FOUNDEROS_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".founderos", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "FounderOS", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
