/**
 * Skill: `hubspot.log_note`
 *
 * Creates an engagement note in HubSpot and associates it with a contact.
 * Permission-gated identically to other Wave 18A HubSpot action skills.
 *
 * Permission semantics:
 *   - `observe`    → skill throws; nothing is written.
 *   - `draft`      → no external call; creates a `pending_approval` row.
 *   - `approve`    → same as `draft`.
 *   - `autonomous` → creates the note and association; logs
 *                    `hubspot.log_note_executed` on success and
 *                    `hubspot.log_note_failed` on error.
 */

import { and, eq } from "drizzle-orm";
import type { Db } from "@founderos/db";
import { integrations, approvals } from "@founderos/db";
import type { AgentPermissionLevel } from "@founderos/shared";
import { createHubspotClient, HubspotAuthError } from "../hubspot-client.js";
import { logActivity } from "../activity-log.js";
import { decryptWithMasterKey } from "../../secrets/local-encrypted-provider.js";
import { logger } from "../../middleware/logger.js";
import {
  evaluateComposioRoute,
  runComposioTool,
} from "./composio-skill-bridge.js";

export const HUBSPOT_LOG_NOTE_SKILL_NAME = "hubspot.log_note" as const;

export interface HubspotLogNoteInput {
  contactId: string;
  body: string;
}

export interface HubspotLogNoteContext {
  db: Db;
  companyId: string;
  permissionLevel: AgentPermissionLevel;
  agentId?: string | null;
  runId?: string | null;
  /** Wave 21: FounderOS user id for Composio routing. Optional. */
  userId?: string | null;
}

export type HubspotLogNoteResult =
  | { ok: true; status: "logged"; noteId: string }
  | { ok: true; status: "pending_approval"; approvalId: string }
  | { ok: false; reason: "no_integration"; message: string }
  | { ok: false; reason: "hubspot_error"; message: string }
  | { ok: false; reason: "composio_error"; message: string };

function validateInput(input: HubspotLogNoteInput): void {
  if (typeof input.contactId !== "string" || input.contactId.trim().length === 0) {
    throw new Error("hubspot.log_note: `contactId` is required");
  }
  if (typeof input.body !== "string" || input.body.trim().length === 0) {
    throw new Error("hubspot.log_note: `body` is required");
  }
}

async function createPendingApproval(params: {
  db: Db;
  companyId: string;
  agentId?: string | null;
  input: HubspotLogNoteInput;
  integrationId: string;
}): Promise<string> {
  const { db, companyId, agentId, input, integrationId } = params;
  const [row] = await db
    .insert(approvals)
    .values({
      companyId,
      type: "hubspot.log_note",
      requestedByAgentId: agentId ?? null,
      status: "pending",
      payload: {
        skill: HUBSPOT_LOG_NOTE_SKILL_NAME,
        integrationId,
        contactId: input.contactId,
        body: input.body,
      },
    })
    .returning({ id: approvals.id });
  return row.id;
}

export async function executeHubspotLogNote(
  ctx: HubspotLogNoteContext,
  input: HubspotLogNoteInput,
): Promise<HubspotLogNoteResult> {
  validateInput(input);

  const { db, companyId, permissionLevel, agentId, runId, userId } = ctx;

  if (permissionLevel === "observe") {
    throw new Error(
      `Observe mode: skill "${HUBSPOT_LOG_NOTE_SKILL_NAME}" is not permitted`,
    );
  }

  // ── Wave 21: Composio routing (autonomous only) ────────────────────────
  if (permissionLevel === "autonomous" && userId) {
    const route = await evaluateComposioRoute({
      db,
      companyId,
      userId,
      appName: "hubspot",
    });
    if (route.shouldUse) {
      const composioOutcome = await runComposioTool({
        userId,
        connectedAccountId: route.composioConnectionId,
        toolName: "hubspot_create_note",
        input: { contactId: input.contactId, body: input.body },
      });
      if (composioOutcome.ok) {
        const noteId =
          typeof composioOutcome.output?.id === "string"
            ? (composioOutcome.output.id as string)
            : typeof composioOutcome.output?.noteId === "string"
              ? (composioOutcome.output.noteId as string)
              : "";
        await logActivity(db, {
          companyId,
          actorType: agentId ? "agent" : "system",
          actorId: agentId ?? "system",
          agentId: agentId ?? null,
          runId: runId ?? null,
          action: "hubspot.log_note_executed",
          entityType: "integration",
          entityId: route.composioConnectionId ?? "composio",
          details: {
            skill: HUBSPOT_LOG_NOTE_SKILL_NAME,
            noteId,
            contactId: input.contactId,
            permissionLevel,
            via: "composio",
          },
        }).catch(() => {});
        return { ok: true, status: "logged", noteId };
      }
      await logActivity(db, {
        companyId,
        actorType: agentId ? "agent" : "system",
        actorId: agentId ?? "system",
        agentId: agentId ?? null,
        runId: runId ?? null,
        action: "hubspot.log_note_failed",
        entityType: "integration",
        entityId: route.composioConnectionId ?? "composio",
        details: {
          skill: HUBSPOT_LOG_NOTE_SKILL_NAME,
          contactId: input.contactId,
          error: composioOutcome.message,
          via: "composio",
        },
      }).catch(() => {});
      return {
        ok: false,
        reason: "composio_error",
        message: composioOutcome.message,
      };
    }
  }

  const [integrationRow] = await db
    .select()
    .from(integrations)
    .where(
      and(
        eq(integrations.companyId, companyId),
        eq(integrations.kind, "hubspot"),
      ),
    );

  if (!integrationRow) {
    return {
      ok: false,
      reason: "no_integration",
      message: "HubSpot integration is not connected for this company",
    };
  }

  if (permissionLevel === "draft" || permissionLevel === "approve") {
    const approvalId = await createPendingApproval({
      db,
      companyId,
      agentId,
      input,
      integrationId: integrationRow.id,
    });

    await logActivity(db, {
      companyId,
      actorType: agentId ? "agent" : "system",
      actorId: agentId ?? "system",
      agentId: agentId ?? null,
      runId: runId ?? null,
      action: "hubspot.log_note_pending_approval",
      entityType: "integration",
      entityId: integrationRow.id,
      details: {
        skill: HUBSPOT_LOG_NOTE_SKILL_NAME,
        approvalId,
        contactId: input.contactId,
        permissionLevel,
      },
    }).catch((err) => {
      logger.error(
        { err, companyId, approvalId },
        "hubspot-log-note: failed to log pending approval activity",
      );
    });

    return {
      ok: true,
      status: "pending_approval",
      approvalId,
    };
  }

  if (permissionLevel !== "autonomous") {
    throw new Error(
      `Unknown permission level "${permissionLevel}" for skill "${HUBSPOT_LOG_NOTE_SKILL_NAME}"`,
    );
  }

  if (!integrationRow.encryptedApiKey) {
    return {
      ok: false,
      reason: "no_integration",
      message: "HubSpot integration is missing its access token",
    };
  }

  let accessToken: string;
  try {
    accessToken = decryptWithMasterKey(integrationRow.encryptedApiKey);
  } catch (err) {
    logger.error(
      { err, companyId, integrationId: integrationRow.id },
      "hubspot-log-note: failed to decrypt access token",
    );
    return {
      ok: false,
      reason: "hubspot_error",
      message: "Failed to decrypt HubSpot access token",
    };
  }

  const client = createHubspotClient({ accessToken });

  try {
    const note = await client.createNote({ body: input.body });
    await client.associateNoteToContact(note.id, input.contactId);

    await logActivity(db, {
      companyId,
      actorType: agentId ? "agent" : "system",
      actorId: agentId ?? "system",
      agentId: agentId ?? null,
      runId: runId ?? null,
      action: "hubspot.log_note_executed",
      entityType: "integration",
      entityId: integrationRow.id,
      details: {
        skill: HUBSPOT_LOG_NOTE_SKILL_NAME,
        noteId: note.id,
        contactId: input.contactId,
        permissionLevel,
        via: "native",
      },
    }).catch((err) => {
      logger.error(
        { err, companyId, integrationId: integrationRow.id },
        "hubspot-log-note: failed to log audit entry",
      );
    });

    return {
      ok: true,
      status: "logged",
      noteId: note.id,
    };
  } catch (err: unknown) {
    const message =
      err instanceof HubspotAuthError
        ? "HubSpot access token is invalid or expired"
        : err instanceof Error
          ? err.message
          : "Unknown HubSpot error";

    await logActivity(db, {
      companyId,
      actorType: agentId ? "agent" : "system",
      actorId: agentId ?? "system",
      agentId: agentId ?? null,
      runId: runId ?? null,
      action: "hubspot.log_note_failed",
      entityType: "integration",
      entityId: integrationRow.id,
      details: {
        skill: HUBSPOT_LOG_NOTE_SKILL_NAME,
        contactId: input.contactId,
        error: message,
      },
    }).catch(() => {});

    logger.error(
      { err, companyId, integrationId: integrationRow.id },
      `hubspot-log-note: createNote/associate failed — ${message}`,
    );

    return { ok: false, reason: "hubspot_error", message };
  }
}
