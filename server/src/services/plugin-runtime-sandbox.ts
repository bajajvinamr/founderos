...
import type { FounderOSPluginManifestV1 } from "@founderos/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __founderos_exports: module.exports,
      __founderos_module: module,
      __founderos_require: requireInSandbox,
      __founderos_filename: realPath,
      __founderos_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__founderos_exports, __founderos_module, __founderos_require, __founderos_filename, __founderos_dirname)`;
