 ...
 */
import { pluginManifestV1Schema } from "@founderos/shared";
import type { FounderOSPluginManifestV1 } from "@founderos/shared";
import { PLUGIN_API_VERSION } from "@founderos/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): FounderOSPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as FounderOSPluginManifestV1,
        };
      }
