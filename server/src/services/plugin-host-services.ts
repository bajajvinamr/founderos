...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@founderos/db";
 ...
 *
 * This implementation delegates to the core FounderOS domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@founderos/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
