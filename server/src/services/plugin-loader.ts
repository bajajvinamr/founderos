 ...
 *    (`~/.founderos/plugins/`) and `node_modules` for packages matching
 *    the `founderos-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published FounderOS plugins.
 * Packages matching this pattern are considered FounderOS plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "founderos-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: FounderOSPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.founderos/plugins/ local directory
  | "npm"               // npm packages matching founderos-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: FounderOSPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the founderos-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "founderos-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: FounderOSPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover FounderOS plugins installed as npm packages in the current
   * Node.js environment matching the "founderos-plugin-*" naming convention.
   *
   ...
   * the "founderosPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a FounderOS plugin.
   * Throws if the package is a FounderOS plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<FounderOSPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: FounderOSPluginManifestV1;
    newManifest: FounderOSPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the FounderOS plugin naming convention.
 * Accepts both the "founderos-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @founderos/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "founderosPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const founderosPlugin = pkgJson["founderosPlugin"];
  if (
    founderosPlugin !== null &&
    typeof founderosPlugin === "object" &&
    !Array.isArray(founderosPlugin)
  ) {
    const manifestRelPath = (founderosPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: FounderOSPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumFounderOSVersion;
}
 ...
 *   packageName: "founderos-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: FounderOSPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as FounderOSPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a FounderOS plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<FounderOSPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a FounderOS plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasFounderOSPlugin = "founderosPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasFounderOSPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<FounderOSPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: FounderOSPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @founderos/shared exports). Run those workers through
