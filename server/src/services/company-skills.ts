...
import { companySkills } from "@founderos/db";
import { readFounderOSSkillSyncPreference } from "@founderos/adapter-utils/server-utils";
import type { FounderOSSkillEntry } from "@founderos/adapter-utils/server-utils";
import type {
  CompanySkill,
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("founderos-ai/founderos/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveFounderOSInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "founderos_bundled") {
    return {
      editable: false,
      editableReason: "Bundled FounderOS skills are read-only.",
      sourceLabel: "FounderOS bundled",
      sourceBadge: "founderos",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "FounderOS workspace",
        sourceBadge: "founderos",
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "founderos_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled FounderOS skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "founderos_bundled"
        && incomingKind === "github"
        && incomingOwner === "founderos"
        && incomingRepo === "founderos"
      ) {
        out.push(existing);
        continue;
      }
