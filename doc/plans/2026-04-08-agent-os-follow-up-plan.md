...
Turn the `agent-os` research into a low-risk FounderOS execution plan that preserves FounderOS's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

FounderOS should not absorb `agent-os` as a product model or orchestration layer.

FounderOS should evaluate `agent-os` in three narrow areas:
...
This work is successful when FounderOS has:
...
- a proposed runtime capability model that fits current FounderOS adapters
...
- replace FounderOS heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to FounderOS core

## Existing FounderOS integration points
...
- prove that FounderOS can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep FounderOS's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a FounderOS-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some FounderOS workloads
...
- identify where FounderOS should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in FounderOS
...
- runtime experimentation could blur the boundary between FounderOS as control plane and FounderOS as execution platform
