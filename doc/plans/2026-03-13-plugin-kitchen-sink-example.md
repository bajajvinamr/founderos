...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented FounderOS plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside FounderOS.
...
- package: `@founderos/plugin-kitchen-sink-example`
- plugin id: `founderos.kitchen-sink-example` or `founderos-kitchen-sink-example`

Recommendation: use `founderos-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core FounderOS event such as `issue.created`
...
Recommendation: default to a FounderOS-local endpoint or a stable public echo endpoint to avoid flaky docs.
