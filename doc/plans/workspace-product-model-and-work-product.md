...
FounderOS needs to support two very different but equally valid ways of working:
...
Today, FounderOS already has the beginnings of this model:
...
- making FounderOS feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing FounderOS users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted FounderOS deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning FounderOS into a full code review product
...
This object must be recorded explicitly so that FounderOS can:
...
FounderOS should treat PRs as a type of work product linked back to:
...
FounderOS already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted FounderOS must be a first-class deployment mode

FounderOS cannot assume that it is running on the same machine as the code.

In cloud deployments, FounderOS may:
...
- `Runtime service`: a process or service FounderOS owns or tracks for a workspace
...
It also lets cloud-hosted FounderOS deployments point at codebases and remotes without pretending the FounderOS host has direct filesystem access.
...
Without an explicit `execution workspace` record, FounderOS has nowhere stable to attach:
...
FounderOS needs a single place to show:
...
- FounderOS-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without FounderOS owning the execution host
...
## 1. FounderOS host and execution host must be decoupled
...
without the FounderOS host itself running local git or local preview processes.
...
FounderOS should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for FounderOS.
...
FounderOS should keep workspace automation centered on a small set of hooks:
...
FounderOS should provide the equivalent visibility in the project `Code` surface:
...
FounderOS should treat execution workspaces as the natural home for:
...
FounderOS should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
FounderOS should continue treating cleanup policy as part of the core workspace model:
...
FounderOS should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the FounderOS host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

FounderOS should borrow Worktrunk's good execution patterns while keeping the broader FounderOS model:
...
This is how FounderOS supports:
...
That keeps FounderOS focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted FounderOS deployments remain first-class

That is a better fit for FounderOS than either extreme:
