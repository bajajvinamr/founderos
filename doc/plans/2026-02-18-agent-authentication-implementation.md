...
- New config/env settings:
  ...
  - `FOUNDEROS_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `FOUNDEROS_AGENT_JWT_ISSUER` (default: `founderos`)
  - `FOUNDEROS_AGENT_JWT_AUDIENCE` (default: `founderos-api`)
...
1. In:
   ...
   inject `FOUNDEROS_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `FOUNDEROS_API_KEY`, do not overwrite it.
- Continue injecting:
  - `FOUNDEROS_AGENT_ID`
...
- Local adapters authenticate without manual `FOUNDEROS_API_KEY` config.
