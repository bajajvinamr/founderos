...
This document defines the product and UI plan for skill management in FounderOS.
...
- FounderOS company import/export can include skills as package content
...
3. The UI must always tell the truth about what FounderOS knows:
   - desired state in FounderOS
...
FounderOS should treat skills at two scopes:
...
- created locally inside FounderOS later
...
7. “Understand what FounderOS knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a FounderOS-specific package layout
...
This matters because Codex and similar adapters may already have local skills that FounderOS did not install.

These should be clearly marked:

- external
- not managed by FounderOS
...
- `desired_only`: FounderOS wants it, adapter does not show it yet
- `external`: adapter has it but FounderOS does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, FounderOS only tracks desired state
...
- FounderOS can still track desired skills
...
- FounderOS can see adapter skills
...
- `.founderos.yaml` may add FounderOS-specific fidelity, but should not replace the base shortname association model
...
- `.founderos.yaml` may add FounderOS-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
FounderOS should have a company-scoped skill model or managed package model representing:
...
FounderOS should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too FounderOS-specific.
...
That gives FounderOS one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
