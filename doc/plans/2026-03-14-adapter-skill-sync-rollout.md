...
This document defines the rollout plan for adapter-wide skill support in FounderOS.
...
- FounderOS stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

FounderOS currently has these adapters:
...
1. Desired skills live in FounderOS for every adapter.
...
These adapters have a stable local skills directory that FounderOS can read and manage.
...
These adapters do not have a meaningful FounderOS-owned persistent install state.
...
- desired skills stored in FounderOS
...
- runtime already injects FounderOS skills into `~/.cursor/skills`
...
- runtime already injects FounderOS skills into `~/.gemini/skills`
...
- runtime already injects FounderOS skills into `~/.pi/agent/skills`
...
- FounderOS can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects FounderOS skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- FounderOS may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed FounderOS skills that are clearly marked as FounderOS-managed
...
- FounderOS stores desired skills only
...
- avoid destructive wording unless FounderOS can prove a skill is FounderOS-managed
...
That gets FounderOS from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
