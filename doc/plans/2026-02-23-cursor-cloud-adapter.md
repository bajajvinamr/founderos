...
This document defines the V1 design for a FounderOS adapter that integrates with
...
4. synthesized stdout events for FounderOS UI/CLI
...
3. **Agent callback auth to FounderOS** uses a bootstrap exchange flow (no long-lived FounderOS key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from FounderOS endpoints, not full SKILL.md prompt injection.
...
- `founderosPublicUrl` (optional override; else `FOUNDEROS_PUBLIC_URL` env)
...
## FounderOS Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `FOUNDEROS_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.founderosPublicUrl`
2. `process.env.FOUNDEROS_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived FounderOS credentials in prompt text.

Flow:

1. Before launch/follow-up, FounderOS mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. FounderOS validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all FounderOS API calls.
...
1. Prompt includes a compact instruction to fetch skills from FounderOS.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/founderos`
   - `GET /api/skills/founderos-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <founderosPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active FounderOS run context.
...
5. `founderosPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from FounderOS skill endpoints |
| FounderOS auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from FounderOS context.
3. Add richer PR metadata surfacing in FounderOS UI.
