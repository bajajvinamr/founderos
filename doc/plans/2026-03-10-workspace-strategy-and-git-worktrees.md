...
`PAP-447` asks how FounderOS should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same FounderOS instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into FounderOS:

- some operators use FounderOS to manage FounderOS and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as FounderOS
- Claude and Codex expose different built-in affordances, so FounderOS should not overfit to one tool

## Core Product Decision

FounderOS should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by FounderOS
...
- FounderOS should not assume that every adapter can see or use a host filesystem path directly
...
If FounderOS makes worktrees agent-first, it will blur:
...
- FounderOS core resolves and records execution workspace state
...
That gives FounderOS room to support:
...
- FounderOS handles workspace runtime services under the hood using project/workspace policy
...
Once FounderOS is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So FounderOS should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that FounderOS cannot infer
...
- when issue moves to done, should FounderOS auto-commit?
- should it auto-push?
...
FounderOS already has the right foundation for a project-first model.
...
For server-spawned local adapters, FounderOS already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that FounderOS should mirror directly

Implication:

- for `codex_local`, FounderOS should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but FounderOS should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, FounderOS should separate:
...
FounderOS should be able to express intentions such as:
...
These are not specific to FounderOS. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".founderos/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a FounderOS-specific `devServer` flag.
...
FounderOS should distinguish between:
...
FounderOS should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, FounderOS should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to FounderOS
...
For runtime services specifically, FounderOS should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as FounderOS-only dev-server flags
...
3. Inject resolved execution workspace details into `context.founderosWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / FounderOS instance can be reused through env/config injection when appropriate
...
- FounderOS can decide whether to reuse or start a fresh service deterministically
...
3. Test against FounderOS-on-FounderOS workflows first.
...
7. Workspace runtime services are modeled generically, not as FounderOS-specific dev-server toggles.
