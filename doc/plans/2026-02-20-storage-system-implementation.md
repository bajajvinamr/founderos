...
Add a single storage subsystem for FounderOS that supports:
...
- API serves attachment bytes through authenticated FounderOS endpoints.
...
- Default local path is under instance root: `~/.founderos/instances/<instanceId>/data/storage`.
...
- `founderos onboard` writes a valid `storage` config block by default.
- `founderos configure --section storage` can switch between local and s3 modes.
...
- `founderos doctor` reports actionable storage status.
