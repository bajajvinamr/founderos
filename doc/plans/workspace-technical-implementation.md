...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/founderos-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `FOUNDEROS_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `FOUNDEROS_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - FounderOS may create host-local worktrees, processes, and paths
- `cloud`
  - FounderOS should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `founderos | github | vercel | s3 | custom`
...
- use `FOUNDEROS_EXECUTION_TOPOLOGY` as a defaulting guardrail
