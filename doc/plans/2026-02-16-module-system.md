...
FounderOS's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends FounderOS's API, UI, and data model | Yes |
...
modules/
  observability/
    founderos.module.json     # manifest (required)
    src/
...
### Manifest (`founderos.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@founderos/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@founderos/core";
...
```
1. Scan modules/ directory for founderos.module.json manifests
...
```jsonc
// founderos.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@founderos/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@founderos/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:founderos/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.founderos.ai/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm founderos store list                    # browse available modules and templates
pnpm founderos store install <module-id>     # install a module
pnpm founderos store import <template-id>    # import a company template
pnpm founderos store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one FounderOS instance | `server:started` |
...
4. **Module config** — `founderos.config.json` with per-module config, env var interpolation
...
10. **`@founderos/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
