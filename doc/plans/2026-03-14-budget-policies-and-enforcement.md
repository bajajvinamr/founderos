...
FounderOS already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model FounderOS should implement next.

## Product Goals

FounderOS should let operators:
...
For the next implementation pass, FounderOS should enforce these defaults:
...
FounderOS should separate subscription-included usage from billed spend:
...
FounderOS should have two threshold classes:
...
FounderOS must know that a project is budget-paused and enforce it.
...
FounderOS must also block execution before new work starts.
