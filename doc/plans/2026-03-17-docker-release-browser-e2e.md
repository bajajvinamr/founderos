...
Today release smoke testing for published FounderOS packages is manual and shell-driven:
...
- Docker
- `npx founderos@canary`
- `npx founderos@latest`
...
- runs `founderos@${FOUNDEROSAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@founderos.local`
- password: `founderos-smoke-password`
...
- the suite passes locally against both:
  - `FOUNDEROSAI_VERSION=canary`
  - `FOUNDEROSAI_VERSION=latest`
...
1. A published `founderos@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `founderos@latest` Docker install can be smoke-tested by Playwright in CI.
