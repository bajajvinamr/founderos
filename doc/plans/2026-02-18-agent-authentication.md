...
Agents need API keys to authenticate with FounderOS. The current approach
...
**Trust model:** The adapter process runs on the same machine as the FounderOS
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** FounderOS generates a token and passes it directly to the agent
...
`FOUNDEROS_API_URL`, `FOUNDEROS_AGENT_ID`, `FOUNDEROS_COMPANY_ID`. We need to
add a `FOUNDEROS_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a FounderOS CLI
...
```
founderos auth login
# Opens browser -> user confirms -> token stored at ~/.founderos/credentials
```
...
managed by the FounderOS server itself.
...
1. A company admin (user or agent) generates an **invite URL** from FounderOS.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The FounderOS SKILL.md (or a link to it)
   - What information FounderOS needs from the agent (e.g. webhook URL, adapter
...
5. FounderOS stores the pending registration.
...
7. On approval, FounderOS provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** FounderOS issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.founderos.ai/skills/founderos/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How FounderOS should send you heartbeats",
...
1. Generate a FounderOS invite link for the company.
...
4. A FounderOS company member approves the new agent.
5. FounderOS begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `FOUNDEROS_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `founderos auth login` for developer-managed remote agents.                                      |
