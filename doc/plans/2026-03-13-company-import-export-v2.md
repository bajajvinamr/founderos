...
This document defines the next-stage plan for FounderOS company import/export.

The core shift is:

- move from a FounderOS-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside FounderOS.
...
FounderOS already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just FounderOS
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `founderos.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.founderos.yaml` sidecar for high-fidelity FounderOS-specific details
...
11. adapter-aware skill sync surfaces so FounderOS can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point FounderOS at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a FounderOS sidecar.
...
FounderOS must not redefine `SKILL.md`.
...
- FounderOS-specific fidelity lives in `.founderos.yaml`
- FounderOS may resolve and install `SKILL.md` packages, but it must not require a FounderOS-only skill format
...
### 5.4 Base Package Vs FounderOS Extension
...
- base package:
  ...
  - no FounderOS-only runtime fields by default
- FounderOS extension:
  - `.founderos.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by FounderOS tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`founderos.manifest.json` is not part of the future package direction.
...
FounderOS import/export should support these entity kinds:
...
- FounderOS can treat “this set of agents came from team package X” as the imported-team model
...
FounderOS should support source references in package metadata with:
...
FounderOS should define a new adapter surface area around skills:
...
Planned FounderOS behavior:

- if an adapter supports read, FounderOS should show current skills in the UI
- if an adapter supports write, FounderOS should let the user enable/disable imported skills
- if an adapter supports sync, FounderOS should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.founderos.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.founderos.yaml` alongside the base package
...
## 10. Storage Model Inside FounderOS
...
FounderOS should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `founderos company export <company-id> --out <path>`
- `founderos company import <path-or-url> --dry-run`
- `founderos company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in FounderOS storage, or only referenced at import time?
...
3. Should FounderOS support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps FounderOS aligned with:
