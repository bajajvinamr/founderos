...
Low-level reference for how FounderOS packages are prepared and published to npm.
...
FounderOS no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `founderos`, imports code from workspace packages such as:
...
### Manual first publish for `@founderos/ui`
...
- If `npm view @founderos/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@founderos/ui`, the `founderos` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
### Manual first publish for `@founderos/mcp-server`
...
```bash
# optional sanity check: this 404s until the first publish exists
npm view @founderos/mcp-server version
...
- If `npm view @founderos/mcp-server version` already returns the same version that is in [`packages/mcp-server/package.json`](../packages/mcp-server/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
- The same npm-side prerequisites apply as above: valid npm auth, permission to publish to the `@founderos` scope, `--access public`, and the required publish auth/2FA policy.

## Version formats

FounderOS uses calendar versions:
...
- `founderos@2026.318.1-canary.2`

This keeps the default install path unchanged while allowing explicit installs with:

```bash
npx founderos@canary onboard
```
