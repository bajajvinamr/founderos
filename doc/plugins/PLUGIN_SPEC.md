# FounderOS Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for FounderOS's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main FounderOS app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary FounderOS HTTP APIs directly.
...
FounderOS plugin design is based on the following assumptions:

1. FounderOS is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core FounderOS business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by FounderOS core.
...
2. Let plugins add major capabilities without editing FounderOS core.
...
4. Depend on project-local plugin folders such as `.founderos/plugins`.
...
The single FounderOS deployment an operator installs and controls.

### 5.2 Company

A first-class FounderOS business object inside the instance.
...
A trusted in-process extension loaded directly by FounderOS core.
...
An installable instance-wide extension package loaded through the FounderOS plugin runtime.
...
FounderOS has two extension classes.
...
FounderOS already has a concrete workspace model:
...
Plugins live under the FounderOS instance directory.

Suggested layout:

- `~/.founderos/instances/default/plugins/package.json`
- `~/.founderos/instances/default/plugins/node_modules/`
- `~/.founderos/instances/default/plugins/.cache/`
- `~/.founderos/instances/default/data/plugins/<plugin-id>/`
...
FounderOS should add CLI commands:

- `pnpm founderos plugin list`
- `pnpm founderos plugin install <package[@version]>`
- `pnpm founderos plugin uninstall <plugin-id>`
- `pnpm founderos plugin upgrade <plugin-id> [version]`
- `pnpm founderos plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@founderos/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface FounderOSPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumFounderOSVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that FounderOS agents can use during runs.
...
- FounderOS server starts one worker process per installed plugin
...
Receives one typed FounderOS domain event.
...
export function definePlugin(definition: PluginDefinition): FounderOSPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: FounderOSPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@founderos/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@founderos/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@founderos/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@founderos/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@founderos/plugin-sdk/ui`
...
The `@founderos/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the FounderOS secret provider system rather than a plain text input
...
1. Core FounderOS data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing FounderOS objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual FounderOS product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if FounderOS later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing FounderOS secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm founderos plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the FounderOS server. This is a normative requirement, not optional.
...
### 27.1 `@founderos/plugin-test-harness`
...
```ts
import { createTestHarness } from "@founderos/plugin-test-harness";
...
For developing a plugin against a running FounderOS instance:

- The operator installs the plugin from a local path: `pnpm founderos plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-founderos-plugin`) that scaffolds:

- `package.json` with correct `founderosPlugin` keys
...
- `@founderos/plugin-workspace-files`
- `@founderos/plugin-terminal`
- `@founderos/plugin-git`
- `@founderos/plugin-linear`
- `@founderos/plugin-github-issues`
- `@founderos/plugin-grafana`
- `@founderos/plugin-runtime-processes`
- `@founderos/plugin-stripe`
...
- `@founderos/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@founderos/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@founderos/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@founderos/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@founderos/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@founderos/plugin-sdk/ui`
...
- `@founderos/plugin-test-harness`
- `create-founderos-plugin` starter template
...
FounderOS should not implement a generic in-process hook bag modeled directly after local coding tools.

FounderOS should implement:
...
That is the complete target design for the FounderOS plugin system.
