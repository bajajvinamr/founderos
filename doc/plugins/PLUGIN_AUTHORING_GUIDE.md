...
This guide describes the current, implemented way to create a FounderOS plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main FounderOS app.
...
```bash
pnpm --filter @founderos/create-founderos-plugin build
node packages/plugins/create-founderos-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the FounderOS repo:

```bash
pnpm --filter @founderos/create-founderos-plugin build
node packages/plugins/create-founderos-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@founderos/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@founderos/plugin-sdk` from the local FounderOS checkout into a `.founderos-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into FounderOS from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@founderos/plugin-sdk/ui`
