...
FounderOS supports two runtime modes:
...
FounderOS now treats **bind** as a separate concern from auth:
...
pnpm founderos onboard --yes --bind lan
pnpm founderos run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, FounderOS emits a startup warning with a one-time high-entropy claim URL.
