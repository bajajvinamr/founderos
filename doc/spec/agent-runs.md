...
This spec defines how FounderOS actually runs agents while staying runtime-agnostic.
...
1. FounderOS is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, FounderOS manages lifecycle, and on exit FounderOS parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `founderos.skill` (shared FounderOS skill text block)
...
2. Preferred transport is env vars (`FOUNDEROS_*`) injected at runtime.
