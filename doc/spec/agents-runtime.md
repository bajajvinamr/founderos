...
For `claude_local` and `codex_local`, FounderOS assumes the CLI is already installed and authenticated on the host machine.
...
FounderOS stores resumable session state per `(agent, taskKey, adapterType)`.
