...
ClipHub is the public registry where people share, discover, and download FounderOS company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to FounderOS what a package registry is to a programming language. FounderOS already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in FounderOS — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own FounderOS instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the FounderOS spec. It contains:
...
**Install (fresh start):**
```
founderos install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local FounderOS instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within FounderOS, export your company as a template, then publish:

```
founderos export --template my-company
founderos publish cliphub my-company
```
...
ClipHub is a **separate service** from FounderOS itself. FounderOS is self-hosted; ClipHub is a hosted registry that FounderOS instances talk to.
...
| **FounderOS CLI** | `founderos install`, `founderos publish`, `founderos cliphub sync` — built into FounderOS |
| **FounderOS UI** | "Browse ClipHub" panel in the FounderOS web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with FounderOS) |
| Backend | TypeScript + Hono (consistent with FounderOS) |
| Database | PostgreSQL |
...
4. Run `founderos install cliphub:acme/lean-saas-shop`
5. FounderOS creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in FounderOS until it works well
2. Export: `founderos export --template my-agency`
3. Publish: `founderos publish cliphub my-agency`
...
3. Install just that agent: `founderos install cliphub:acme/senior-python-eng --agent`
...
ClipHub is **not required** to use FounderOS. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to FounderOS what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `founderos install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in FounderOS web UI
- [ ] `founderos cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with FounderOS's own philosophy)
